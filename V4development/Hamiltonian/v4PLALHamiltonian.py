...
def v4Plalham_and_csi(m1,m2,tortoise,x1,x2,x3,p1,p2,p3,S1x,S1y,S1z,S2x,S2y,S2z)
    ...
    chi2=chi*chi
    chi3=chi2*chi
    third=np.true_divide(1.,3.)
    fifth=np.true_divide(1.,5.)
    ...
    k5=m1PlusEtaKK*m1PlusEtaKK*(-4237./60.+128./5.*EMgamma+2275.*np.pi*np.pi/512.-third*(a*a)*(k1p3-3.*(k1*k2)+3.*k3)-((k1p3*k1p2)-5.*(k1p3*k2)+5.*k1*k2*k2+5.*k1p2*k3-5.*k2*k3-5.*k1*k4)*fifth*invm1PlusEtaKK*invm1PlusEtaKK+((k1p2*k1p2)-4.*(k1p2*k2)+2.*k2*k2+4.*k1*k3-4.*k4)*0.5*invm1PlusEtaKK+(256./5.)*ln2+(41.*np.pi*np.pi/32.-221./6.)*eta)
    ...
    logTerms=1.+eta*k0+eta*np.log(np.abs(1.+logarg))
    deltaU=np.abs(bulk*logTerms)
    deltaT=r2*deltaU
    ...
    D=1.+np.log(1+6.*eta*u2+2.*(26.-3.*eta)*eta*u3)
    deltaR=deltaT*D
    qq=2.*eta*(4.-3.*eta)
    ww=2.*a*r
    csi=np.sqrt(np.abs(deltaT*deltaR))/w2
    ...
    Hns=np.sqrt((1.+((prT*prT)*(prT*prT))*qq*u2+ptheta2*invrho2+pf*pf*rho2*invLambda*invxi2+pr*pr*deltaR*invrho2)*(rho2*deltaT)*invLambda)+pf*ww*invLambda
    B=np.sqrt(deltaT)
    sqrtdeltaT=B
    ...
    w=ww*invLambda
    expnu=np.sqrt(deltaT*rho2*invLambda)
    expMU=np.sqrt(rho2)
    ...
    Hreal=np.sqrt(1.+2.*eta*(np.abs(H)-1.))
    return [Hreal,csi]
