...
double ham_x_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    ...
    double u = 1.0/r;
    double sigmastar3 = (S1z*m2/m1 + S2z*m1/m2)/pow(M, 2);
    double sigmastar2 = (S1y*m2/m1 + S2y*m1/m2)/pow(M, 2);
    double sigmastar1 = (S1x*m2/m1 + S2x*m1/m2)/pow(M, 2);
    double sigma3 = (S1z + S2z)/pow(M, 2);
    double sigma2 = (S1y + S2y)/pow(M, 2);
    double sigma1 = (S1x + S2x)/pow(M, 2);
    ...
    double n2 = y/r;
    double n1 = x/r;
    double TINYDOUBLE = 1.0e-100;
    double condition_e3prov_lhs = a;
    double condition_e3prov_rhs = 9.9999999999999998e-17;
    double e3prov_gt_bound = ((1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs + (1.0/2.0)*fabs(condition_e3prov_lhs - condition_e3prov_rhs))/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov_leq_bound = (-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs))/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov1 = Skerrhat1*e3prov_gt_bound + 0.57735026918962584*e3prov_leq_bound;
    double e3prov2 = Skerrhat2*e3prov_gt_bound + 0.57735026918962584*e3prov_leq_bound;
    double e3prov3 = Skerrhat3*e3prov_gt_bound + 0.57735026918962584*e3prov_leq_bound;
    double lambdavec3 = Lhat1*n2 - Lhat2*n3;
    double lambdavec2 = -Lhat1*n3 + Lhat3*n1;
    double lambdavec1 = Lhat2*n3 - Lhat3*n2;
    double lambdavecnorm = sqrt(pow(lambdavec1, 2) + pow(lambdavec2, 2) + pow(lambdavec3, 2));
    ...
    double lambdahat_dot_e3prov = e3prov1*lambdahat1 + e3prov2*lambdahat2 + e3prov3*lambdahat3;
    double lambdahat_cross_e3prov3 = -e3prov1*lambdahat2 + e3prov2*lambdahat1;
    double lambdahat_cross_e3prov2 = e3prov1*lambdahat3 - e3prov3*lambdahat1;
    double lambdahat_cross_e3prov1 = -e3prov2*lambdahat3 + e3prov3*lambdahat2;
    double e3prov_dot_n = e3prov1*n1 + e3prov2*n2 + e3prov3*n3;
    double cos_0_1_deg = 0.99999838000043741;
    double sin_0_1_deg = 0.0017999990280001574;
    double condition_e3_lhs = 1 - fabs(e3prov_dot_n);
    double condition_e3_rhs = 1.0e-8;
    double e3_gt_bound = ((1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs + (1.0/2.0)*fabs(condition_e3_lhs - condition_e3_rhs))/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e3_leq_bound = (-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3_lhs + condition_e3_rhs))/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e31 = e3_gt_bound*e3prov1 + e3_leq_bound*(cos_0_1_deg*e3prov1 + lambdahat1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1*sin_0_1_deg);
    double e32 = e3_gt_bound*e3prov2 + e3_leq_bound*(cos_0_1_deg*e3prov2 + lambdahat2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2*sin_0_1_deg);
    double e33 = e3_gt_bound*e3prov3 + e3_leq_bound*(cos_0_1_deg*e3prov3 + lambdahat3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3*sin_0_1_deg);
    ...
    double Dinvprime = -pow(u, 2)*(eta*pow(u, 2)*(156 - 18*eta) + 12*eta*u)/(eta*pow(u, 3)*(52 - 6*eta) + 6*eta*pow(u, 2) + 1);
    ...
    double Deltarprime = Deltat*Dinvprime + Deltatprime*Dinv;
    double Lambt = fabs(Deltat*pow(a, 2)*sin2theta - pow(w2, 2));
    double csi = sqrt(fabs(Deltar*Deltat))/w2;
    double csiprime = -2.0*r*sqrt(Deltar*Deltat)/pow(w2, 2) + (1.0/2.0)*(Deltar*Deltatprime + Deltarprime*Deltat)/(w2*sqrt(Deltar*Deltat));
    ...
    double sigmacoeffTerm2 = eta*(eta*(r*(-6.125*DrSipn2*pow(u, 2) + 1.4166666666666667*Qminus1*pow(u, 2) + r*(5.625*pow(DrSipn2, 2)*pow(u, 2) - 1.625*DrSipn2*Qminus1*pow(u, 2))) - 2.3333333333333335*pow(u, 2)) + r*(-0.66666666666666663*DrSipn2*pow(u, 2) - 3.0277777777777777*Qminus1*pow(u, 2) + r*(0.25*DrSipn2*Qminus1*pow(u, 2) - 0.3125*pow(Qminus1, 2)*pow(u, 2))) - 6.2222222222222223*pow(u, 2));
    ...
    double HsoTerm2coeff = expnu/(Deltat*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1 = Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq);
    ...
    double lambdavec3_prmx = Lhat1*n2_prmx + Lhat1_prmx*n2 - Lhat2*n3_prmx - Lhat2_prmx*n3;
    double lambdavec2_prmx = -Lhat1*n3_prmx - Lhat1_prmx*n3 + Lhat3*n1_prmx + Lhat3_prmx*n1;
    double lambdavec1_prmx = Lhat2*n3_prmx + Lhat2_prmx*n3 - Lhat3*n2_prmx - Lhat3_prmx*n2;
    double lambdavecnorm_prmx = (lambdavec1*lambdavec1_prmx + lambdavec2*lambdavec2_prmx + lambdavec3*lambdavec3_prmx)/sqrt(pow(lambdavec1, 2) + pow(lambdavec2, 2) + pow(lambdavec3, 2));
    double lambdahat3_prmx = -lambdavec3*lambdavecnorm_prmx/pow(lambdavecnorm, 2) + lambdavec3_prmx/lambdavecnorm;
    double lambdahat2_prmx = -lambdavec2*lambdavecnorm_prmx/pow(lambdavecnorm, 2) + lambdavec2_prmx/lambdavecnorm;
    double lambdahat1_prmx = -lambdavec1*lambdavecnorm_prmx/pow(lambdavecnorm, 2) + lambdavec1_prmx/lambdavecnorm;
    double lambdahat_dot_e3prov_prmx = e3prov1*lambdahat1_prmx + e3prov2*lambdahat2_prmx + e3prov3*lambdahat3_prmx;
    double lambdahat_cross_e3prov3_prmx = -e3prov1*lambdahat2_prmx + e3prov2*lambdahat1_prmx;
    double lambdahat_cross_e3prov2_prmx = e3prov1*lambdahat3_prmx - e3prov3*lambdahat1_prmx;
    double lambdahat_cross_e3prov1_prmx = -e3prov2*lambdahat3_prmx + e3prov3*lambdahat2_prmx;
    double e3prov_dot_n_prmx = e3prov1*n1_prmx + e3prov2*n2_prmx + e3prov3*n3_prmx;
    double condition_e3_lhs_prmx = -e3prov_dot_n_prmx*(((e3prov_dot_n) > 0) - ((e3prov_dot_n) < 0));
    double e3_gt_bound_prmx = -condition_e3_lhs_prmx*((1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs + (1.0/2.0)*fabs(condition_e3_lhs - condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmx*(((condition_e3_lhs - condition_e3_rhs) > 0) - ((condition_e3_lhs - condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmx)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e3_leq_bound_prmx = -condition_e3_lhs_prmx*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3_lhs + condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmx*(((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) > 0) - ((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmx)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e31_prmx = e3_gt_bound_prmx*e3prov1 + e3_leq_bound*(lambdahat1*lambdahat_dot_e3prov_prmx*(1 - cos_0_1_deg) + lambdahat1_prmx*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmx*sin_0_1_deg) + e3_leq_bound_prmx*(cos_0_1_deg*e3prov1 + lambdahat1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1*sin_0_1_deg);
    double e32_prmx = e3_gt_bound_prmx*e3prov2 + e3_leq_bound*(lambdahat2*lambdahat_dot_e3prov_prmx*(1 - cos_0_1_deg) + lambdahat2_prmx*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmx*sin_0_1_deg) + e3_leq_bound_prmx*(cos_0_1_deg*e3prov2 + lambdahat2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2*sin_0_1_deg);
    double e33_prmx = e3_gt_bound_prmx*e3prov3 + e3_leq_bound*(lambdahat3*lambdahat_dot_e3prov_prmx*(1 - cos_0_1_deg) + lambdahat3_prmx*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmx*sin_0_1_deg) + e3_leq_bound_prmx*(cos_0_1_deg*e3prov3 + lambdahat3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3*sin_0_1_deg);
    double xi3_prmx = e31*n2_prmx + e31_prmx*n2 - e32*n1_prmx - e32_prmx*n1;
    double xi2_prmx = -e31*n3_prmx - e31_prmx*n3 + e33*n1_prmx + e33_prmx*n1;
    double xi1_prmx = e32*n3_prmx + e32_prmx*n3 - e33*n2_prmx - e33_prmx*n2;
    ...
    double costheta_prmx = e31*n1_prmx + e31_prmx*n1 + e32*n2_prmx + e32_prmx*n2 + e33*n3_prmx + e33_prmx*n3;
    ...
    double Dinvprime_prmx = -pow(u, 2)*(eta*pow(u, 2)*(156 - 18*eta) + 12*eta*u)*(eta*pow(u, 2)*u_prmx*(18*eta - 156) - 12*eta*u*u_prmx)/pow(eta*pow(u, 3)*(52 - 6*eta) + 6*eta*pow(u, 2) + 1, 2) - pow(u, 2)*(eta*u*u_prmx*(312 - 36*eta) + 12*eta*u_prmx)/(eta*pow(u, 3)*(52 - 6*eta) + 6*eta*pow(u, 2) + 1) + u*u_prmx*(-2*eta*pow(u, 2)*(156 - 18*eta) - 24*eta*u)/(eta*pow(u, 3)*(52 - 6*eta) + 6*eta*pow(u, 2) + 1);
    ...
    double Deltarprime_prmx = Deltat*Dinvprime_prmx + Deltat_prmx*Dinvprime + Deltatprime*Dinv_prmx + Deltatprime_prmx*Dinv;
    ...
    double csiprime_prmx = 4.0*r*w2_prmx*sqrt(Deltar*Deltat)/pow(w2, 3) - 2.0*r_prmx*sqrt(Deltar*Deltat)/pow(w2, 2) + (1.0/2.0)*(Deltar*Deltatprime_prmx + Deltar_prmx*Deltatprime + Deltarprime*Deltat_prmx + Deltarprime_prmx*Deltat)/(w2*sqrt(Deltar*Deltat)) - 1.0/2.0*w2_prmx*(Deltar*Deltatprime + Deltarprime*Deltat)/(pow(w2, 2)*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmx + (1.0/2.0)*Deltar_prmx*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmx - 1.0/2.0*Deltar_prmx*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double sigmacoeffTerm2_prmx = eta*(eta*(r*(-12.25*DrSipn2*u*u_prmx - 6.125*DrSipn2_prmx*pow(u, 2) + 2.8333333333333299*Qminus1*u*u_prmx + 1.4166666666666701*Qminus1_prmx*pow(u, 2) + r*(11.25*pow(DrSipn2, 2)*u*u_prmx + 11.25*DrSipn2*DrSipn2_prmx*pow(u, 2) - 3.25*DrSipn2*Qminus1*u*u_prmx - 1.625*DrSipn2*Qminus1_prmx*pow(u, 2) - 1.625*DrSipn2_prmx*Qminus1*pow(u, 2)) + r_prmx*(5.625*pow(DrSipn2, 2)*pow(u, 2) - 1.625*DrSipn2*Qminus1*pow(u, 2))) + r_prmx*(-6.125*DrSipn2*pow(u, 2) + 1.4166666666666701*Qminus1*pow(u, 2) + r*(5.625*pow(DrSipn2, 2)*pow(u, 2) - 1.625*DrSipn2*Qminus1*pow(u, 2))) - 4.6666666666666696*u*u_prmx) + r*(-1.3333333333333299*DrSipn2*u*u_prmx - 0.66666666666666696*DrSipn2_prmx*pow(u, 2) - 6.0555555555555598*Qminus1*u*u_prmx - 3.0277777777777799*Qminus1_prmx*pow(u, 2) + r*(0.5*DrSipn2*Qminus1*u*u_prmx + 0.25*DrSipn2*Qminus1_prmx*pow(u, 2) + 0.25*DrSipn2_prmx*Qminus1*pow(u, 2) - 0.625*pow(Qminus1, 2)*u*u_prmx - 0.625*Qminus1*Qminus1_prmx*pow(u, 2)) + r_prmx*(0.25*DrSipn2*Qminus1*pow(u, 2) - 0.3125*pow(Qminus1, 2)*pow(u, 2))) + r_prmx*(-0.66666666666666696*DrSipn2*pow(u, 2) - 3.0277777777777799*Qminus1*pow(u, 2) + r*(0.25*DrSipn2*Qminus1*pow(u, 2) - 0.3125*pow(Qminus1, 2)*pow(u, 2))) - 12.4444444444444*u*u_prmx);
    ...
    double Sdote3_prmx = S1*e31_prmx + S1_prmx*e31 + S2*e32_prmx + S2_prmx*e32 + S3*e33_prmx + S3_prmx*e33;
    ...
    double HssTerm1_prmx = Sdote3*omega_prmx + Sdote3_prmx*omega;
    ...
    double HsoTerm2coeff_prmx = expnu*(-Q_prmx - 1.0/2.0*Q_prmx/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmx/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmx/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmx*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmx*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmx = Sdote3*exp2nu*pdotxir*(-Btilde_prmx + expmu*expnu_prmx + expmu_prmx*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmx*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmx*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmx*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmx*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmx*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmx*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmx*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_y_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double lambdavec3_prmy = Lhat1*n2_prmy + Lhat1_prmy*n2 - Lhat2*n3_prmy - Lhat2_prmy*n3;
    double lambdavec2_prmy = -Lhat1*n3_prmy - Lhat1_prmy*n3 + Lhat3*n1_prmy + Lhat3_prmy*n1;
    double lambdavec1_prmy = Lhat2*n3_prmy + Lhat2_prmy*n3 - Lhat3*n2_prmy - Lhat3_prmy*n2;
    double lambdavecnorm_prmy = (lambdavec1*lambdavec1_prmy + lambdavec2*lambdavec2_prmy + lambdavec3*lambdavec3_prmy)/sqrt(pow(lambdavec1, 2) + pow(lambdavec2, 2) + pow(lambdavec3, 2));
    double lambdahat3_prmy = -lambdavec3*lambdavecnorm_prmy/pow(lambdavecnorm, 2) + lambdavec3_prmy/lambdavecnorm;
    double lambdahat2_prmy = -lambdavec2*lambdavecnorm_prmy/pow(lambdavecnorm, 2) + lambdavec2_prmy/lambdavecnorm;
    double lambdahat1_prmy = -lambdavec1*lambdavecnorm_prmy/pow(lambdavecnorm, 2) + lambdavec1_prmy/lambdavecnorm;
    double lambdahat_dot_e3prov_prmy = e3prov1*lambdahat1_prmy + e3prov2*lambdahat2_prmy + e3prov3*lambdahat3_prmy;
    double lambdahat_cross_e3prov3_prmy = -e3prov1*lambdahat2_prmy + e3prov2*lambdahat1_prmy;
    double lambdahat_cross_e3prov2_prmy = e3prov1*lambdahat3_prmy - e3prov3*lambdahat1_prmy;
    double lambdahat_cross_e3prov1_prmy = -e3prov2*lambdahat3_prmy + e3prov3*lambdahat2_prmy;
    double e3prov_dot_n_prmy = e3prov1*n1_prmy + e3prov2*n2_prmy + e3prov3*n3_prmy;
    double condition_e3_lhs_prmy = -e3prov_dot_n_prmy*(((e3prov_dot_n) > 0) - ((e3prov_dot_n) < 0));
    double e3_gt_bound_prmy = -condition_e3_lhs_prmy*((1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs + (1.0/2.0)*fabs(condition_e3_lhs - condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmy*(((condition_e3_lhs - condition_e3_rhs) > 0) - ((condition_e3_lhs - condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmy)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e3_leq_bound_prmy = -condition_e3_lhs_prmy*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3_lhs + condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmy*(((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) > 0) - ((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmy)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e31_prmy = e3_gt_bound_prmy*e3prov1 + e3_leq_bound*(lambdahat1*lambdahat_dot_e3prov_prmy*(1 - cos_0_1_deg) + lambdahat1_prmy*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmy*sin_0_1_deg) + e3_leq_bound_prmy*(cos_0_1_deg*e3prov1 + lambdahat1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1*sin_0_1_deg);
    double e32_prmy = e3_gt_bound_prmy*e3prov2 + e3_leq_bound*(lambdahat2*lambdahat_dot_e3prov_prmy*(1 - cos_0_1_deg) + lambdahat2_prmy*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmy*sin_0_1_deg) + e3_leq_bound_prmy*(cos_0_1_deg*e3prov2 + lambdahat2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2*sin_0_1_deg);
    double e33_prmy = e3_gt_bound_prmy*e3prov3 + e3_leq_bound*(lambdahat3*lambdahat_dot_e3prov_prmy*(1 - cos_0_1_deg) + lambdahat3_prmy*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmy*sin_0_1_deg) + e3_leq_bound_prmy*(cos_0_1_deg*e3prov3 + lambdahat3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3*sin_0_1_deg);
    double xi3_prmy = e31*n2_prmy + e31_prmy*n2 - e32*n1_prmy - e32_prmy*n1;
    double xi2_prmy = -e31*n3_prmy - e31_prmy*n3 + e33*n1_prmy + e33_prmy*n1;
    double xi1_prmy = e32*n3_prmy + e32_prmy*n3 - e33*n2_prmy - e33_prmy*n2;
    ...
    double costheta_prmy = e31*n1_prmy + e31_prmy*n1 + e32*n2_prmy + e32_prmy*n2 + e33*n3_prmy + e33_prmy*n3;
    ...
    double Dinvprime_prmy = -pow(u, 2)*(eta*pow(u, 2)*(156 - 18*eta) + 12*eta*u)*(eta*pow(u, 2)*u_prmy*(18*eta - 156) - 12*eta*u*u_prmy)/pow(eta*pow(u, 3)*(52 - 6*eta) + 6*eta*pow(u, 2) + 1, 2) - pow(u, 2)*(eta*u*u_prmy*(312 - 36*eta) + 12*eta*u_prmy)/(eta*pow(u, 3)*(52 - 6*eta) + 6*eta*pow(u, 2) + 1) + u*u_prmy*(-2*eta*pow(u, 2)*(156 - 18*eta) - 24*eta*u)/(eta*pow(u, 3)*(52 - 6*eta) + 6*eta*pow(u, 2) + 1);
    ...
    double Deltarprime_prmy = Deltat*Dinvprime_prmy + Deltat_prmy*Dinvprime + Deltatprime*Dinv_prmy + Deltatprime_prmy*Dinv;
    ...
    double csiprime_prmy = 4.0*r*w2_prmy*sqrt(Deltar*Deltat)/pow(w2, 3) - 2.0*r_prmy*sqrt(Deltar*Deltat)/pow(w2, 2) + (1.0/2.0)*(Deltar*Deltatprime_prmy + Deltar_prmy*Deltatprime + Deltarprime*Deltat_prmy + Deltarprime_prmy*Deltat)/(w2*sqrt(Deltar*Deltat)) - 1.0/2.0*w2_prmy*(Deltar*Deltatprime + Deltarprime*Deltat)/(pow(w2, 2)*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmy + (1.0/2.0)*Deltar_prmy*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmy - 1.0/2.0*Deltar_prmy*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double sigmacoeffTerm2_prmy = eta*(eta*(r*(-12.25*DrSipn2*u*u_prmy - 6.125*DrSipn2_prmy*pow(u, 2) + 2.8333333333333299*Qminus1*u*u_prmy + 1.4166666666666701*Qminus1_prmy*pow(u, 2) + r*(11.25*pow(DrSipn2, 2)*u*u_prmy + 11.25*DrSipn2*DrSipn2_prmy*pow(u, 2) - 3.25*DrSipn2*Qminus1*u*u_prmy - 1.625*DrSipn2*Qminus1_prmy*pow(u, 2) - 1.625*DrSipn2_prmy*Qminus1*pow(u, 2)) + r_prmy*(5.625*pow(DrSipn2, 2)*pow(u, 2) - 1.625*DrSipn2*Qminus1*pow(u, 2))) + r_prmy*(-6.125*DrSipn2*pow(u, 2) + 1.4166666666666701*Qminus1*pow(u, 2) + r*(5.625*pow(DrSipn2, 2)*pow(u, 2) - 1.625*DrSipn2*Qminus1*pow(u, 2))) - 4.6666666666666696*u*u_prmy) + r*(-1.3333333333333299*DrSipn2*u*u_prmy - 0.66666666666666696*DrSipn2_prmy*pow(u, 2) - 6.0555555555555598*Qminus1*u*u_prmy - 3.0277777777777799*Qminus1_prmy*pow(u, 2) + r*(0.5*DrSipn2*Qminus1*u*u_prmy + 0.25*DrSipn2*Qminus1_prmy*pow(u, 2) + 0.25*DrSipn2_prmy*Qminus1*pow(u, 2) - 0.625*pow(Qminus1, 2)*u*u_prmy - 0.625*Qminus1*Qminus1_prmy*pow(u, 2)) + r_prmy*(0.25*DrSipn2*Qminus1*pow(u, 2) - 0.3125*pow(Qminus1, 2)*pow(u, 2))) + r_prmy*(-0.66666666666666696*DrSipn2*pow(u, 2) - 3.0277777777777799*Qminus1*pow(u, 2) + r*(0.25*DrSipn2*Qminus1*pow(u, 2) - 0.3125*pow(Qminus1, 2)*pow(u, 2))) - 12.4444444444444*u*u_prmy);
    ...
    double Sdote3_prmy = S1*e31_prmy + S1_prmy*e31 + S2*e32_prmy + S2_prmy*e32 + S3*e33_prmy + S3_prmy*e33;
    ...
    double HssTerm1_prmy = Sdote3*omega_prmy + Sdote3_prmy*omega;
    ...
    double HsoTerm2coeff_prmy = expnu*(-Q_prmy - 1.0/2.0*Q_prmy/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmy/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmy/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmy*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmy*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmy = Sdote3*exp2nu*pdotxir*(-Btilde_prmy + expmu*expnu_prmy + expmu_prmy*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmy*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmy*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmy*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmy*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmy*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmy*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmy*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_z_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double lambdavec3_prmz = Lhat1*n2_prmz + Lhat1_prmz*n2 - Lhat2*n3_prmz - Lhat2_prmz*n3;
    double lambdavec2_prmz = -Lhat1*n3_prmz - Lhat1_prmz*n3 + Lhat3*n1_prmz + Lhat3_prmz*n1;
    double lambdavec1_prmz = Lhat2*n3_prmz + Lhat2_prmz*n3 - Lhat3*n2_prmz - Lhat3_prmz*n2;
    double lambdavecnorm_prmz = (lambdavec1*lambdavec1_prmz + lambdavec2*lambdavec2_prmz + lambdavec3*lambdavec3_prmz)/sqrt(pow(lambdavec1, 2) + pow(lambdavec2, 2) + pow(lambdavec3, 2));
    double lambdahat3_prmz = -lambdavec3*lambdavecnorm_prmz/pow(lambdavecnorm, 2) + lambdavec3_prmz/lambdavecnorm;
    double lambdahat2_prmz = -lambdavec2*lambdavecnorm_prmz/pow(lambdavecnorm, 2) + lambdavec2_prmz/lambdavecnorm;
    double lambdahat1_prmz = -lambdavec1*lambdavecnorm_prmz/pow(lambdavecnorm, 2) + lambdavec1_prmz/lambdavecnorm;
    double lambdahat_dot_e3prov_prmz = e3prov1*lambdahat1_prmz + e3prov2*lambdahat2_prmz + e3prov3*lambdahat3_prmz;
    double lambdahat_cross_e3prov3_prmz = -e3prov1*lambdahat2_prmz + e3prov2*lambdahat1_prmz;
    double lambdahat_cross_e3prov2_prmz = e3prov1*lambdahat3_prmz - e3prov3*lambdahat1_prmz;
    double lambdahat_cross_e3prov1_prmz = -e3prov2*lambdahat3_prmz + e3prov3*lambdahat2_prmz;
    double e3prov_dot_n_prmz = e3prov1*n1_prmz + e3prov2*n2_prmz + e3prov3*n3_prmz;
    double condition_e3_lhs_prmz = -e3prov_dot_n_prmz*(((e3prov_dot_n) > 0) - ((e3prov_dot_n) < 0));
    double e3_gt_bound_prmz = -condition_e3_lhs_prmz*((1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs + (1.0/2.0)*fabs(condition_e3_lhs - condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmz*(((condition_e3_lhs - condition_e3_rhs) > 0) - ((condition_e3_lhs - condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmz)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e3_leq_bound_prmz = -condition_e3_lhs_prmz*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3_lhs + condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmz*(((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) > 0) - ((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmz)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    ...
    double e32_prmz = e3_gt_bound_prmz*e3prov2 + e3_leq_bound*(lambdahat2*lambdahat_dot_e3prov_prmz*(1 - cos_0_1_deg) + lambdahat2_prmz*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmz*sin_0_1_deg) + e3_leq_bound_prmz*(cos_0_1_deg*e3prov2 + lambdahat2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2*sin_0_1_deg);
    double e33_prmz = e3_gt_bound_prmz*e3prov3 + e3_leq_bound*(lambdahat3*lambdahat_dot_e3prov_prmz*(1 - cos_0_1_deg) + lambdahat3_prmz*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmz*sin_0_1_deg) + e3_leq_bound_prmz*(cos_0_1_deg*e3prov3 + lambdahat3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3*sin_0_1_deg);
    double xi3_prmz = e31*n2_prmz + e31_prmz*n2 - e32*n1_prmz - e32_prmz*n1;
    double xi2_prmz = -e31*n3_prmz - e31_prmz*n3 + e33*n1_prmz + e33_prmz*n1;
    double xi1_prmz = e32*n3_prmz + e32_prmz*n3 - e33*n2_prmz - e33_prmz*n2;
    ...
    double costheta_prmz = e31*n1_prmz + e31_prmz*n1 + e32*n2_prmz + e32_prmz*n2 + e33*n3_prmz + e33_prmz*n3;
    ...
    double Dinvprime_prmz = -pow(u, 2)*(eta*pow(u, 2)*(156 - 18*eta) + 12*eta*u)*(eta*pow(u, 2)*u_prmz*(18*eta - 156) - 12*eta*u*u_prmz)/pow(eta*pow(u, 3)*(52 - 6*eta) + 6*eta*pow(u, 2) + 1, 2) - pow(u, 2)*(eta*u*u_prmz*(312 - 36*eta) + 12*eta*u_prmz)/(eta*pow(u, 3)*(52 - 6*eta) + 6*eta*pow(u, 2) + 1) + u*u_prmz*(-2*eta*pow(u, 2)*(156 - 18*eta) - 24*eta*u)/(eta*pow(u, 3)*(52 - 6*eta) + 6*eta*pow(u, 2) + 1);
    ...
    double Deltarprime_prmz = Deltat*Dinvprime_prmz + Deltat_prmz*Dinvprime + Deltatprime*Dinv_prmz + Deltatprime_prmz*Dinv;
    ...
    double csiprime_prmz = 4.0*r*w2_prmz*sqrt(Deltar*Deltat)/pow(w2, 3) - 2.0*r_prmz*sqrt(Deltar*Deltat)/pow(w2, 2) + (1.0/2.0)*(Deltar*Deltatprime_prmz + Deltar_prmz*Deltatprime + Deltarprime*Deltat_prmz + Deltarprime_prmz*Deltat)/(w2*sqrt(Deltar*Deltat)) - 1.0/2.0*w2_prmz*(Deltar*Deltatprime + Deltarprime*Deltat)/(pow(w2, 2)*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmz + (1.0/2.0)*Deltar_prmz*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmz - 1.0/2.0*Deltar_prmz*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double sigmacoeffTerm2_prmz = eta*(eta*(r*(-12.25*DrSipn2*u*u_prmz - 6.125*DrSipn2_prmz*pow(u, 2) + 2.8333333333333299*Qminus1*u*u_prmz + 1.4166666666666701*Qminus1_prmz*pow(u, 2) + r*(11.25*pow(DrSipn2, 2)*u*u_prmz + 11.25*DrSipn2*DrSipn2_prmz*pow(u, 2) - 3.25*DrSipn2*Qminus1*u*u_prmz - 1.625*DrSipn2*Qminus1_prmz*pow(u, 2) - 1.625*DrSipn2_prmz*Qminus1*pow(u, 2)) + r_prmz*(5.625*pow(DrSipn2, 2)*pow(u, 2) - 1.625*DrSipn2*Qminus1*pow(u, 2))) + r_prmz*(-6.125*DrSipn2*pow(u, 2) + 1.4166666666666701*Qminus1*pow(u, 2) + r*(5.625*pow(DrSipn2, 2)*pow(u, 2) - 1.625*DrSipn2*Qminus1*pow(u, 2))) - 4.6666666666666696*u*u_prmz) + r*(-1.3333333333333299*DrSipn2*u*u_prmz - 0.66666666666666696*DrSipn2_prmz*pow(u, 2) - 6.0555555555555598*Qminus1*u*u_prmz - 3.0277777777777799*Qminus1_prmz*pow(u, 2) + r*(0.5*DrSipn2*Qminus1*u*u_prmz + 0.25*DrSipn2*Qminus1_prmz*pow(u, 2) + 0.25*DrSipn2_prmz*Qminus1*pow(u, 2) - 0.625*pow(Qminus1, 2)*u*u_prmz - 0.625*Qminus1*Qminus1_prmz*pow(u, 2)) + r_prmz*(0.25*DrSipn2*Qminus1*pow(u, 2) - 0.3125*pow(Qminus1, 2)*pow(u, 2))) + r_prmz*(-0.66666666666666696*DrSipn2*pow(u, 2) - 3.0277777777777799*Qminus1*pow(u, 2) + r*(0.25*DrSipn2*Qminus1*pow(u, 2) - 0.3125*pow(Qminus1, 2)*pow(u, 2))) - 12.4444444444444*u*u_prmz);
    ...
    double Sdote3_prmz = S1*e31_prmz + S1_prmz*e31 + S2*e32_prmz + S2_prmz*e32 + S3*e33_prmz + S3_prmz*e33;
    ...
    double HssTerm1_prmz = Sdote3*omega_prmz + Sdote3_prmz*omega;
    ...
    double HsoTerm2coeff_prmz = expnu*(-Q_prmz - 1.0/2.0*Q_prmz/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmz/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmz/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmz*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmz*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmz = Sdote3*exp2nu*pdotxir*(-Btilde_prmz + expmu*expnu_prmz + expmu_prmz*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmz*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmz*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmz*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmz*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmz*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmz*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmz*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_p1_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double lambdavec3_prmp1 = Lhat1_prmp1*n2 - Lhat2_prmp1*n3;
    double lambdavec2_prmp1 = -Lhat1_prmp1*n3 + Lhat3_prmp1*n1;
    double lambdavec1_prmp1 = Lhat2_prmp1*n3 - Lhat3_prmp1*n2;
    double lambdavecnorm_prmp1 = (lambdavec1*lambdavec1_prmp1 + lambdavec2*lambdavec2_prmp1 + lambdavec3*lambdavec3_prmp1)/sqrt(pow(lambdavec1, 2) + pow(lambdavec2, 2) + pow(lambdavec3, 2));
    double lambdahat3_prmp1 = -lambdavec3*lambdavecnorm_prmp1/pow(lambdavecnorm, 2) + lambdavec3_prmp1/lambdavecnorm;
    double lambdahat2_prmp1 = -lambdavec2*lambdavecnorm_prmp1/pow(lambdavecnorm, 2) + lambdavec2_prmp1/lambdavecnorm;
    double lambdahat1_prmp1 = -lambdavec1*lambdavecnorm_prmp1/pow(lambdavecnorm, 2) + lambdavec1_prmp1/lambdavecnorm;
    double lambdahat_dot_e3prov_prmp1 = e3prov1*lambdahat1_prmp1 + e3prov2*lambdahat2_prmp1 + e3prov3*lambdahat3_prmp1;
    double lambdahat_cross_e3prov3_prmp1 = -e3prov1*lambdahat2_prmp1 + e3prov2*lambdahat1_prmp1;
    double lambdahat_cross_e3prov2_prmp1 = e3prov1*lambdahat3_prmp1 - e3prov3*lambdahat1_prmp1;
    double lambdahat_cross_e3prov1_prmp1 = -e3prov2*lambdahat3_prmp1 + e3prov3*lambdahat2_prmp1;
    double e31_prmp1 = e3_leq_bound*(lambdahat1*lambdahat_dot_e3prov_prmp1*(1 - cos_0_1_deg) + lambdahat1_prmp1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmp1*sin_0_1_deg);
    double e32_prmp1 = e3_leq_bound*(lambdahat2*lambdahat_dot_e3prov_prmp1*(1 - cos_0_1_deg) + lambdahat2_prmp1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmp1*sin_0_1_deg);
    double e33_prmp1 = e3_leq_bound*(lambdahat3*lambdahat_dot_e3prov_prmp1*(1 - cos_0_1_deg) + lambdahat3_prmp1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmp1*sin_0_1_deg);
    double xi3_prmp1 = e31_prmp1*n2 - e32_prmp1*n1;
    double xi2_prmp1 = -e31_prmp1*n3 + e33_prmp1*n1;
    double xi1_prmp1 = e32_prmp1*n3 - e33_prmp1*n2;
    double v3_prmp1 = n1*xi2_prmp1 - n2*xi1_prmp1;
    double v2_prmp1 = -n1*xi3_prmp1 + n3*xi1_prmp1;
    double v1_prmp1 = n2*xi3_prmp1 - n3*xi2_prmp1;
    double costheta_prmp1 = e31_prmp1*n1 + e32_prmp1*n2 + e33_prmp1*n3;
    double sin2theta_prmp1 = -2*costheta*costheta_prmp1;
    double xisq_prmp1 = sin2theta_prmp1;
    double Sigma_prmp1 = 2*pow(a, 2)*costheta*costheta_prmp1;
    ...
    double Deltarprime_prmp1 = Deltat_prmp1*Dinvprime + Deltatprime_prmp1*Dinv;
    double Lambt_prmp1 = (Deltat*pow(a, 2)*sin2theta_prmp1 + Deltat_prmp1*pow(a, 2)*sin2theta)*(((Deltat*pow(a, 2)*sin2theta - pow(w2, 2)) > 0) - ((Deltat*pow(a, 2)*sin2theta - pow(w2, 2)) < 0));
    ...
    double csiprime_prmp1 = (1.0/2.0)*(Deltar*Deltatprime_prmp1 + Deltar_prmp1*Deltatprime + Deltarprime*Deltat_prmp1 + Deltarprime_prmp1*Deltat)/(w2*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmp1 + (1.0/2.0)*Deltar_prmp1*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmp1 - 1.0/2.0*Deltar_prmp1*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double pdotxir_prmp1 = r*(phat1*xi1_prmp1 + phat1_prmp1*xi1 + phat2*xi2_prmp1 + phat2_prmp1*xi2 + phat3*xi3_prmp1 + phat3_prmp1*xi3);
    double pdotn_prmp1 = n1*phat1_prmp1 + n2*phat2_prmp1 + n3*phat3_prmp1;
    double pdotvr_prmp1 = r*(phat1*v1_prmp1 + phat1_prmp1*v1 + phat2*v2_prmp1 + phat2_prmp1*v2 + phat3*v3_prmp1 + phat3_prmp1*v3);
    double pphi_prmp1 = pdotxir_prmp1;
    double Qcoeff2_prmp1 = -sin2theta_prmp1/(Sigma*pow(sin2theta, 2)) - Sigma_prmp1/(pow(Sigma, 2)*sin2theta);
    double Qcoeff1_prmp1 = -Sigma*sin2theta_prmp1/(Lambt*pow(sin2theta, 2)) + Sigma_prmp1/(Lambt*sin2theta) - Lambt_prmp1*Sigma/(pow(Lambt, 2)*sin2theta);
    double DrSipn2_prmp1 = 2*Deltar*pdotn*pdotn_prmp1/Sigma - Deltar*Sigma_prmp1*pow(pdotn, 2)/pow(Sigma, 2) + Deltar_prmp1*pow(pdotn, 2)/Sigma;
    double Q_prmp1 = DrSipn2_prmp1 + 2*Qcoeff1*pdotxir*pdotxir_prmp1 + Qcoeff1_prmp1*pow(pdotxir, 2) + 2*Qcoeff2*pdotvr*pdotvr_prmp1 + Qcoeff2_prmp1*pow(pdotvr, 2);
    double Qminus1_prmp1 = Q_prmp1;
    double Jtilde_prmp1 = (1.0/2.0)*Deltar_prmp1/sqrt(Deltar);
    double exp2mu_prmp1 = Sigma_prmp1;
    double expmu_prmp1 = (1.0/2.0)*exp2mu_prmp1/sqrt(exp2mu);
    ...
    double exp2nu_prmp1 = Deltat*Sigma_prmp1/Lambt - Deltat*Lambt_prmp1*Sigma/pow(Lambt, 2) + Deltat_prmp1*Sigma/Lambt;
    double expnu_prmp1 = (1.0/2.0)*exp2nu_prmp1/sqrt(exp2nu);
    double omega_prmp1 = -Lambt_prmp1*omegatilde/pow(Lambt, 2);
    double Lambtprime_prmp1 = -Deltatprime*pow(a, 2)*sin2theta_prmp1 - Deltatprime_prmp1*pow(a, 2)*sin2theta;
    double mucostheta_prmp1 = pow(a, 2)*costheta_prmp1/Sigma - Sigma_prmp1*pow(a, 2)*costheta/pow(Sigma, 2);
    double nucostheta_prmp1 = -Deltat_prmp1*pow(a, 2)*costheta*w2/(Lambt*Sigma) + pow(a, 2)*costheta_prmp1*w2*(-Deltat + w2)/(Lambt*Sigma) - Sigma_prmp1*pow(a, 2)*costheta*w2*(-Deltat + w2)/(Lambt*pow(Sigma, 2)) - Lambt_prmp1*pow(a, 2)*costheta*w2*(-Deltat + w2)/(pow(Lambt, 2)*Sigma);
    double omegacostheta_prmp1 = -2*Deltat*pow(a, 2)*costheta_prmp1*omegatilde/pow(Lambt, 2) + 4*Deltat*Lambt_prmp1*pow(a, 2)*costheta*omegatilde/pow(Lambt, 3) - 2*Deltat_prmp1*pow(a, 2)*costheta*omegatilde/pow(Lambt, 2);
    double mur_prmp1 = -Sigma_prmp1*r/pow(Sigma, 2) + (1.0/2.0)*Deltar_prmp1/pow(Deltar, 3.0/2.0);
    double nur_prmp1 = -Sigma_prmp1*r/pow(Sigma, 2) + (1.0/2.0)*w2*(-4*Deltat_prmp1*r + Deltatprime_prmp1*w2)/(Deltat*Lambt) - 1.0/2.0*Lambt_prmp1*w2*(-4*Deltat*r + Deltatprime*w2)/(Deltat*pow(Lambt, 2)) - 1.0/2.0*Deltat_prmp1*w2*(-4*Deltat*r + Deltatprime*w2)/(pow(Deltat, 2)*Lambt);
    ...
    double sigmacoeffTerm2_prmp1 = eta*(eta*r*(-6.125*DrSipn2_prmp1*pow(u, 2) + 1.4166666666666701*Qminus1_prmp1*pow(u, 2) + r*(11.25*DrSipn2*DrSipn2_prmp1*pow(u, 2) - 1.625*DrSipn2*Qminus1_prmp1*pow(u, 2) - 1.625*DrSipn2_prmp1*Qminus1*pow(u, 2))) + r*(-0.66666666666666696*DrSipn2_prmp1*pow(u, 2) - 3.0277777777777799*Qminus1_prmp1*pow(u, 2) + r*(0.25*DrSipn2*Qminus1_prmp1*pow(u, 2) + 0.25*DrSipn2_prmp1*Qminus1*pow(u, 2) - 0.625*Qminus1*Qminus1_prmp1*pow(u, 2))));
    ...
    double Sdote3_prmp1 = S1*e31_prmp1 + S1_prmp1*e31 + S2*e32_prmp1 + S2_prmp1*e32 + S3*e33_prmp1 + S3_prmp1*e33;
    double Sdotn_prmp1 = S1_prmp1*n1 + S2_prmp1*n2 + S3_prmp1*n3;
    double Sdotv_prmp1 = S1*v1_prmp1 + S1_prmp1*v1 + S2*v2_prmp1 + S2_prmp1*v2 + S3*v3_prmp1 + S3_prmp1*v3;
    double Sdotxi_prmp1 = S1*xi1_prmp1 + S1_prmp1*xi1 + S2*xi2_prmp1 + S2_prmp1*xi2 + S3*xi3_prmp1 + S3_prmp1*xi3;
    ...
    double HsoTerm2a_prmp1 = pow(Btilde, 2)*Jtilde*Sdotxi*(-sqrt(Q)*(nur*pdotvr_prmp1 + nur_prmp1*pdotvr + pdotn*xisq*(mucostheta_prmp1 - nucostheta_prmp1) + pdotn*xisq_prmp1*(mucostheta - nucostheta) + pdotn_prmp1*xisq*(mucostheta - nucostheta)) - mucostheta*pdotn*xisq_prmp1 - mucostheta*pdotn_prmp1*xisq - mucostheta_prmp1*pdotn*xisq + mur*pdotvr_prmp1*(sqrt(Q) + 1) + mur_prmp1*pdotvr*(sqrt(Q) + 1) + (1.0/2.0)*Q_prmp1*mur*pdotvr/sqrt(Q) + (1.0/2.0)*Q_prmp1*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta))/sqrt(Q)) + pow(Btilde, 2)*Jtilde*Sdotxi_prmp1*(sqrt(Q)*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta)) - mucostheta*pdotn*xisq + mur*pdotvr*(sqrt(Q) + 1)) + pow(Btilde, 2)*Jtilde_prmp1*Sdotxi*(sqrt(Q)*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta)) - mucostheta*pdotn*xisq + mur*pdotvr*(sqrt(Q) + 1)) + Btilde*Btilde_prmp1*Jtilde*Sdotxi*(2*sqrt(Q)*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta)) - 2*mucostheta*pdotn*xisq + 2*mur*pdotvr*(sqrt(Q) + 1));
    double HsoTerm2_prmp1 = HsoTerm2a_prmp1 + HsoTerm2b_prmp1 - HsoTerm2c_prmp1;
    double HsoTerm2coeff_prmp1 = expnu*(-Q_prmp1 - 1.0/2.0*Q_prmp1/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmp1/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmp1/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmp1*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmp1*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmp1 = Sdote3*exp2nu*pdotxir*(-Btilde_prmp1 + expmu*expnu_prmp1 + expmu_prmp1*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmp1*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmp1*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmp1*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmp1*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmp1*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmp1*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmp1*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_p2_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double lambdavec3_prmp2 = Lhat1_prmp2*n2 - Lhat2_prmp2*n3;
    double lambdavec2_prmp2 = -Lhat1_prmp2*n3 + Lhat3_prmp2*n1;
    double lambdavec1_prmp2 = Lhat2_prmp2*n3 - Lhat3_prmp2*n2;
    double lambdavecnorm_prmp2 = (lambdavec1*lambdavec1_prmp2 + lambdavec2*lambdavec2_prmp2 + lambdavec3*lambdavec3_prmp2)/sqrt(pow(lambdavec1, 2) + pow(lambdavec2, 2) + pow(lambdavec3, 2));
    double lambdahat3_prmp2 = -lambdavec3*lambdavecnorm_prmp2/pow(lambdavecnorm, 2) + lambdavec3_prmp2/lambdavecnorm;
    double lambdahat2_prmp2 = -lambdavec2*lambdavecnorm_prmp2/pow(lambdavecnorm, 2) + lambdavec2_prmp2/lambdavecnorm;
    double lambdahat1_prmp2 = -lambdavec1*lambdavecnorm_prmp2/pow(lambdavecnorm, 2) + lambdavec1_prmp2/lambdavecnorm;
    double lambdahat_dot_e3prov_prmp2 = e3prov1*lambdahat1_prmp2 + e3prov2*lambdahat2_prmp2 + e3prov3*lambdahat3_prmp2;
    double lambdahat_cross_e3prov3_prmp2 = -e3prov1*lambdahat2_prmp2 + e3prov2*lambdahat1_prmp2;
    double lambdahat_cross_e3prov2_prmp2 = e3prov1*lambdahat3_prmp2 - e3prov3*lambdahat1_prmp2;
    double lambdahat_cross_e3prov1_prmp2 = -e3prov2*lambdahat3_prmp2 + e3prov3*lambdahat2_prmp2;
    double e31_prmp2 = e3_leq_bound*(lambdahat1*lambdahat_dot_e3prov_prmp2*(1 - cos_0_1_deg) + lambdahat1_prmp2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmp2*sin_0_1_deg);
    double e32_prmp2 = e3_leq_bound*(lambdahat2*lambdahat_dot_e3prov_prmp2*(1 - cos_0_1_deg) + lambdahat2_prmp2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmp2*sin_0_1_deg);
    double e33_prmp2 = e3_leq_bound*(lambdahat3*lambdahat_dot_e3prov_prmp2*(1 - cos_0_1_deg) + lambdahat3_prmp2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmp2*sin_0_1_deg);
    double xi3_prmp2 = e31_prmp2*n2 - e32_prmp2*n1;
    double xi2_prmp2 = -e31_prmp2*n3 + e33_prmp2*n1;
    double xi1_prmp2 = e32_prmp2*n3 - e33_prmp2*n2;
    double v3_prmp2 = n1*xi2_prmp2 - n2*xi1_prmp2;
    double v2_prmp2 = -n1*xi3_prmp2 + n3*xi1_prmp2;
    double v1_prmp2 = n2*xi3_prmp2 - n3*xi2_prmp2;
    double costheta_prmp2 = e31_prmp2*n1 + e32_prmp2*n2 + e33_prmp2*n3;
    double sin2theta_prmp2 = -2*costheta*costheta_prmp2;
    double xisq_prmp2 = sin2theta_prmp2;
    double Sigma_prmp2 = 2*pow(a, 2)*costheta*costheta_prmp2;
    ...
    double Deltarprime_prmp2 = Deltat_prmp2*Dinvprime + Deltatprime_prmp2*Dinv;
    double Lambt_prmp2 = (Deltat*pow(a, 2)*sin2theta_prmp2 + Deltat_prmp2*pow(a, 2)*sin2theta)*(((Deltat*pow(a, 2)*sin2theta - pow(w2, 2)) > 0) - ((Deltat*pow(a, 2)*sin2theta - pow(w2, 2)) < 0));
    ...
    double csiprime_prmp2 = (1.0/2.0)*(Deltar*Deltatprime_prmp2 + Deltar_prmp2*Deltatprime + Deltarprime*Deltat_prmp2 + Deltarprime_prmp2*Deltat)/(w2*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmp2 + (1.0/2.0)*Deltar_prmp2*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmp2 - 1.0/2.0*Deltar_prmp2*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double pdotxir_prmp2 = r*(phat1*xi1_prmp2 + phat1_prmp2*xi1 + phat2*xi2_prmp2 + phat2_prmp2*xi2 + phat3*xi3_prmp2 + phat3_prmp2*xi3);
    double pdotn_prmp2 = n1*phat1_prmp2 + n2*phat2_prmp2 + n3*phat3_prmp2;
    double pdotvr_prmp2 = r*(phat1*v1_prmp2 + phat1_prmp2*v1 + phat2*v2_prmp2 + phat2_prmp2*v2 + phat3*v3_prmp2 + phat3_prmp2*v3);
    double pphi_prmp2 = pdotxir_prmp2;
    double Qcoeff2_prmp2 = -sin2theta_prmp2/(Sigma*pow(sin2theta, 2)) - Sigma_prmp2/(pow(Sigma, 2)*sin2theta);
    double Qcoeff1_prmp2 = -Sigma*sin2theta_prmp2/(Lambt*pow(sin2theta, 2)) + Sigma_prmp2/(Lambt*sin2theta) - Lambt_prmp2*Sigma/(pow(Lambt, 2)*sin2theta);
    double DrSipn2_prmp2 = 2*Deltar*pdotn*pdotn_prmp2/Sigma - Deltar*Sigma_prmp2*pow(pdotn, 2)/pow(Sigma, 2) + Deltar_prmp2*pow(pdotn, 2)/Sigma;
    double Q_prmp2 = DrSipn2_prmp2 + 2*Qcoeff1*pdotxir*pdotxir_prmp2 + Qcoeff1_prmp2*pow(pdotxir, 2) + 2*Qcoeff2*pdotvr*pdotvr_prmp2 + Qcoeff2_prmp2*pow(pdotvr, 2);
    double Qminus1_prmp2 = Q_prmp2;
    double Jtilde_prmp2 = (1.0/2.0)*Deltar_prmp2/sqrt(Deltar);
    double exp2mu_prmp2 = Sigma_prmp2;
    double expmu_prmp2 = (1.0/2.0)*exp2mu_prmp2/sqrt(exp2mu);
    ...
    double exp2nu_prmp2 = Deltat*Sigma_prmp2/Lambt - Deltat*Lambt_prmp2*Sigma/pow(Lambt, 2) + Deltat_prmp2*Sigma/Lambt;
    double expnu_prmp2 = (1.0/2.0)*exp2nu_prmp2/sqrt(exp2nu);
    double omega_prmp2 = -Lambt_prmp2*omegatilde/pow(Lambt, 2);
    double Lambtprime_prmp2 = -Deltatprime*pow(a, 2)*sin2theta_prmp2 - Deltatprime_prmp2*pow(a, 2)*sin2theta;
    double mucostheta_prmp2 = pow(a, 2)*costheta_prmp2/Sigma - Sigma_prmp2*pow(a, 2)*costheta/pow(Sigma, 2);
    double nucostheta_prmp2 = -Deltat_prmp2*pow(a, 2)*costheta*w2/(Lambt*Sigma) + pow(a, 2)*costheta_prmp2*w2*(-Deltat + w2)/(Lambt*Sigma) - Sigma_prmp2*pow(a, 2)*costheta*w2*(-Deltat + w2)/(Lambt*pow(Sigma, 2)) - Lambt_prmp2*pow(a, 2)*costheta*w2*(-Deltat + w2)/(pow(Lambt, 2)*Sigma);
    double omegacostheta_prmp2 = -2*Deltat*pow(a, 2)*costheta_prmp2*omegatilde/pow(Lambt, 2) + 4*Deltat*Lambt_prmp2*pow(a, 2)*costheta*omegatilde/pow(Lambt, 3) - 2*Deltat_prmp2*pow(a, 2)*costheta*omegatilde/pow(Lambt, 2);
    double mur_prmp2 = -Sigma_prmp2*r/pow(Sigma, 2) + (1.0/2.0)*Deltar_prmp2/pow(Deltar, 3.0/2.0);
    double nur_prmp2 = -Sigma_prmp2*r/pow(Sigma, 2) + (1.0/2.0)*w2*(-4*Deltat_prmp2*r + Deltatprime_prmp2*w2)/(Deltat*Lambt) - 1.0/2.0*Lambt_prmp2*w2*(-4*Deltat*r + Deltatprime*w2)/(Deltat*pow(Lambt, 2)) - 1.0/2.0*Deltat_prmp2*w2*(-4*Deltat*r + Deltatprime*w2)/(pow(Deltat, 2)*Lambt);
    ...
    double sigmacoeffTerm2_prmp2 = eta*(eta*r*(-6.125*DrSipn2_prmp2*pow(u, 2) + 1.4166666666666701*Qminus1_prmp2*pow(u, 2) + r*(11.25*DrSipn2*DrSipn2_prmp2*pow(u, 2) - 1.625*DrSipn2*Qminus1_prmp2*pow(u, 2) - 1.625*DrSipn2_prmp2*Qminus1*pow(u, 2))) + r*(-0.66666666666666696*DrSipn2_prmp2*pow(u, 2) - 3.0277777777777799*Qminus1_prmp2*pow(u, 2) + r*(0.25*DrSipn2*Qminus1_prmp2*pow(u, 2) + 0.25*DrSipn2_prmp2*Qminus1*pow(u, 2) - 0.625*Qminus1*Qminus1_prmp2*pow(u, 2))));
    ...
    double Sdote3_prmp2 = S1*e31_prmp2 + S1_prmp2*e31 + S2*e32_prmp2 + S2_prmp2*e32 + S3*e33_prmp2 + S3_prmp2*e33;
    double Sdotn_prmp2 = S1_prmp2*n1 + S2_prmp2*n2 + S3_prmp2*n3;
    double Sdotv_prmp2 = S1*v1_prmp2 + S1_prmp2*v1 + S2*v2_prmp2 + S2_prmp2*v2 + S3*v3_prmp2 + S3_prmp2*v3;
    double Sdotxi_prmp2 = S1*xi1_prmp2 + S1_prmp2*xi1 + S2*xi2_prmp2 + S2_prmp2*xi2 + S3*xi3_prmp2 + S3_prmp2*xi3;
    ...
    double gammappsum_prmp2 = 2*Deltar*pdotn*pdotn_prmp2/Sigma - Deltar*Sigma_prmp2*pow(pdotn, 2)/pow(Sigma, 2) + Deltar_prmp2*pow(pdotn, 2)/Sigma - pow(pdotvr, 2)*sin2theta_prmp2/(Sigma*pow(sin2theta, 2)) + 2*pdotvr*pdotvr_prmp2/(Sigma*sin2theta) - Sigma_prmp2*pow(pdotvr, 2)/(pow(Sigma, 2)*sin2theta) - Sigma*pow(pdotxir, 2)*sin2theta_prmp2/(Lambt*pow(sin2theta, 2)) + 2*Sigma*pdotxir*pdotxir_prmp2/(Lambt*sin2theta) + Sigma_prmp2*pow(pdotxir, 2)/(Lambt*sin2theta) - Lambt_prmp2*Sigma*pow(pdotxir, 2)/(pow(Lambt, 2)*sin2theta);
    double Hnsradicand_prmp2 = Q4_prmp2 + gammappsum_prmp2;
    double alpha_prmp2 = Lambt*sqrt(Deltat*Sigma/Lambt)*((1.0/2.0)*Deltat*Sigma_prmp2/Lambt - 1.0/2.0*Deltat*Lambt_prmp2*Sigma/pow(Lambt, 2) + (1.0/2.0)*Deltat_prmp2*Sigma/Lambt)/(Deltat*Sigma);
    double betapsum_prmp2 = omegatilde*pphi_prmp2/Lambt - Lambt_prmp2*omegatilde*pphi/pow(Lambt, 2);
    double HssTerm3_prmp2 = pow(Btilde, 2)*(Sdotn*exp2mu*xisq*(-Q_prmp2 - 1.0/2.0*Q_prmp2/sqrt(Q)) + Sdotn*exp2mu*xisq_prmp2*(-sqrt(Q) - Q) + Sdotn*exp2mu_prmp2*xisq*(-sqrt(Q) - Q) + Sdotn_prmp2*exp2mu*xisq*(-sqrt(Q) - Q) + pdotvr*(-Jtilde*Sdotv*pdotn_prmp2 - Jtilde*Sdotv_prmp2*pdotn - Jtilde_prmp2*Sdotv*pdotn + Sdotn*pdotvr_prmp2 + Sdotn_prmp2*pdotvr) + pdotvr_prmp2*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr)) + Btilde*Btilde_prmp2*(-2*Sdotn*exp2mu*xisq*(sqrt(Q) + Q) + 2*pdotvr*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr)) + expmu*expnu*pdotxir*(Btilde*Jtilde*Sdotxi*pdotn_prmp2 + Btilde*Jtilde*Sdotxi_prmp2*pdotn + Btilde*Jtilde_prmp2*Sdotxi*pdotn + Btilde_prmp2*Jtilde*Sdotxi*pdotn - Sdotn*expmu*expnu*pdotxir_prmp2 - Sdotn*expmu*expnu_prmp2*pdotxir - Sdotn*expmu_prmp2*expnu*pdotxir - Sdotn_prmp2*expmu*expnu*pdotxir) + expmu*expnu*pdotxir_prmp2*(Btilde*Jtilde*Sdotxi*pdotn - Sdotn*expmu*expnu*pdotxir) + expmu*expnu_prmp2*pdotxir*(Btilde*Jtilde*Sdotxi*pdotn - Sdotn*expmu*expnu*pdotxir) + expmu_prmp2*expnu*pdotxir*(Btilde*Jtilde*Sdotxi*pdotn - Sdotn*expmu*expnu*pdotxir);
    double HssTerm3coeff_prmp2 = (1.0/2.0)*omegacostheta*(-Q_prmp2 - 1.0/2.0*Q_prmp2/sqrt(Q))/(Btilde*exp2mu*expmu*expnu*pow(sqrt(Q) + Q, 2)) + omegacostheta_prmp2/(Btilde*exp2mu*expmu*expnu*(2*sqrt(Q) + 2*Q)) - expnu_prmp2*omegacostheta/(Btilde*exp2mu*expmu*pow(expnu, 2)*(2*sqrt(Q) + 2*Q)) - expmu_prmp2*omegacostheta/(Btilde*exp2mu*pow(expmu, 2)*expnu*(2*sqrt(Q) + 2*Q)) - exp2mu_prmp2*omegacostheta/(Btilde*pow(exp2mu, 2)*expmu*expnu*(2*sqrt(Q) + 2*Q)) - Btilde_prmp2*omegacostheta/(pow(Btilde, 2)*exp2mu*expmu*expnu*(2*sqrt(Q) + 2*Q));
    double HssTerm2_prmp2 = pow(Btilde, 2)*xisq*(Jtilde*pdotn*(-Jtilde*Sdotv*pdotn_prmp2 - Jtilde*Sdotv_prmp2*pdotn - Jtilde_prmp2*Sdotv*pdotn + Sdotn*pdotvr_prmp2 + Sdotn_prmp2*pdotvr) + Jtilde*pdotn_prmp2*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr) + Jtilde_prmp2*pdotn*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr) + Sdotv*exp2mu*(Q_prmp2 + (1.0/2.0)*Q_prmp2/sqrt(Q)) + Sdotv*exp2mu_prmp2*(sqrt(Q) + Q) + Sdotv_prmp2*exp2mu*(sqrt(Q) + Q)) + pow(Btilde, 2)*xisq_prmp2*(Jtilde*pdotn*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr) + Sdotv*exp2mu*(sqrt(Q) + Q)) + Btilde*Btilde_prmp2*xisq*(2*Jtilde*pdotn*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr) + 2*Sdotv*exp2mu*(sqrt(Q) + Q)) + expmu*pdotxir*(-Btilde*Sdotxi*expnu*pdotvr_prmp2 - Btilde*Sdotxi*expnu_prmp2*pdotvr - Btilde*Sdotxi_prmp2*expnu*pdotvr - Btilde_prmp2*Sdotxi*expnu*pdotvr + Sdotv*exp2nu*expmu*pdotxir_prmp2 + Sdotv*exp2nu*expmu_prmp2*pdotxir + Sdotv*exp2nu_prmp2*expmu*pdotxir + Sdotv_prmp2*exp2nu*expmu*pdotxir) + expmu*pdotxir_prmp2*(-Btilde*Sdotxi*expnu*pdotvr + Sdotv*exp2nu*expmu*pdotxir) + expmu_prmp2*pdotxir*(-Btilde*Sdotxi*expnu*pdotvr + Sdotv*exp2nu*expmu*pdotxir);
    double HssTerm2coeff_prmp2 = (1.0/2.0)*Jtilde*omegar*(-Q_prmp2 - 1.0/2.0*Q_prmp2/sqrt(Q))/(Btilde*exp2mu*expmu*expnu*xisq*pow(sqrt(Q) + Q, 2)) - Jtilde*omegar*xisq_prmp2/(Btilde*exp2mu*expmu*expnu*pow(xisq, 2)*(2*sqrt(Q) + 2*Q)) + Jtilde*omegar_prmp2/(Btilde*exp2mu*expmu*expnu*xisq*(2*sqrt(Q) + 2*Q)) - Jtilde*expnu_prmp2*omegar/(Btilde*exp2mu*expmu*pow(expnu, 2)*xisq*(2*sqrt(Q) + 2*Q)) - Jtilde*expmu_prmp2*omegar/(Btilde*exp2mu*pow(expmu, 2)*expnu*xisq*(2*sqrt(Q) + 2*Q)) - Jtilde*exp2mu_prmp2*omegar/(Btilde*pow(exp2mu, 2)*expmu*expnu*xisq*(2*sqrt(Q) + 2*Q)) + Jtilde_prmp2*omegar/(Btilde*exp2mu*expmu*expnu*xisq*(2*sqrt(Q) + 2*Q)) - Btilde_prmp2*Jtilde*omegar/(pow(Btilde, 2)*exp2mu*expmu*expnu*xisq*(2*sqrt(Q) + 2*Q));
    double HssTerm1_prmp2 = Sdote3*omega_prmp2 + Sdote3_prmp2*omega;
    ...
    double HsoTerm2c_prmp2 = Brtilde*Jtilde*Sdotv*expmu*expnu*pdotxir_prmp2*(sqrt(Q) + 1) + Brtilde*Jtilde*Sdotv*expmu*expnu_prmp2*pdotxir*(sqrt(Q) + 1) + Brtilde*Jtilde*Sdotv*expmu_prmp2*expnu*pdotxir*(sqrt(Q) + 1) + Brtilde*Jtilde*Sdotv_prmp2*expmu*expnu*pdotxir*(sqrt(Q) + 1) + (1.0/2.0)*Brtilde*Jtilde*Q_prmp2*Sdotv*expmu*expnu*pdotxir/sqrt(Q) + Brtilde*Jtilde_prmp2*Sdotv*expmu*expnu*pdotxir*(sqrt(Q) + 1) + Brtilde_prmp2*Jtilde*Sdotv*expmu*expnu*pdotxir*(sqrt(Q) + 1);
    ...
    double HsoTerm2a_prmp2 = pow(Btilde, 2)*Jtilde*Sdotxi*(-sqrt(Q)*(nur*pdotvr_prmp2 + nur_prmp2*pdotvr + pdotn*xisq*(mucostheta_prmp2 - nucostheta_prmp2) + pdotn*xisq_prmp2*(mucostheta - nucostheta) + pdotn_prmp2*xisq*(mucostheta - nucostheta)) - mucostheta*pdotn*xisq_prmp2 - mucostheta*pdotn_prmp2*xisq - mucostheta_prmp2*pdotn*xisq + mur*pdotvr_prmp2*(sqrt(Q) + 1) + mur_prmp2*pdotvr*(sqrt(Q) + 1) + (1.0/2.0)*Q_prmp2*mur*pdotvr/sqrt(Q) + (1.0/2.0)*Q_prmp2*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta))/sqrt(Q)) + pow(Btilde, 2)*Jtilde*Sdotxi_prmp2*(sqrt(Q)*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta)) - mucostheta*pdotn*xisq + mur*pdotvr*(sqrt(Q) + 1)) + pow(Btilde, 2)*Jtilde_prmp2*Sdotxi*(sqrt(Q)*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta)) - mucostheta*pdotn*xisq + mur*pdotvr*(sqrt(Q) + 1)) + Btilde*Btilde_prmp2*Jtilde*Sdotxi*(2*sqrt(Q)*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta)) - 2*mucostheta*pdotn*xisq + 2*mur*pdotvr*(sqrt(Q) + 1));
    double HsoTerm2_prmp2 = HsoTerm2a_prmp2 + HsoTerm2b_prmp2 - HsoTerm2c_prmp2;
    double HsoTerm2coeff_prmp2 = expnu*(-Q_prmp2 - 1.0/2.0*Q_prmp2/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmp2/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmp2/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmp2*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmp2*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmp2 = Sdote3*exp2nu*pdotxir*(-Btilde_prmp2 + expmu*expnu_prmp2 + expmu_prmp2*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmp2*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmp2*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmp2*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmp2*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmp2*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmp2*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmp2*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_p3_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double lambdavec3_prmp3 = Lhat1_prmp3*n2 - Lhat2_prmp3*n3;
    double lambdavec2_prmp3 = -Lhat1_prmp3*n3 + Lhat3_prmp3*n1;
    double lambdavec1_prmp3 = Lhat2_prmp3*n3 - Lhat3_prmp3*n2;
    double lambdavecnorm_prmp3 = (lambdavec1*lambdavec1_prmp3 + lambdavec2*lambdavec2_prmp3 + lambdavec3*lambdavec3_prmp3)/sqrt(pow(lambdavec1, 2) + pow(lambdavec2, 2) + pow(lambdavec3, 2));
    double lambdahat3_prmp3 = -lambdavec3*lambdavecnorm_prmp3/pow(lambdavecnorm, 2) + lambdavec3_prmp3/lambdavecnorm;
    double lambdahat2_prmp3 = -lambdavec2*lambdavecnorm_prmp3/pow(lambdavecnorm, 2) + lambdavec2_prmp3/lambdavecnorm;
    double lambdahat1_prmp3 = -lambdavec1*lambdavecnorm_prmp3/pow(lambdavecnorm, 2) + lambdavec1_prmp3/lambdavecnorm;
    double lambdahat_dot_e3prov_prmp3 = e3prov1*lambdahat1_prmp3 + e3prov2*lambdahat2_prmp3 + e3prov3*lambdahat3_prmp3;
    double lambdahat_cross_e3prov3_prmp3 = -e3prov1*lambdahat2_prmp3 + e3prov2*lambdahat1_prmp3;
    double lambdahat_cross_e3prov2_prmp3 = e3prov1*lambdahat3_prmp3 - e3prov3*lambdahat1_prmp3;
    double lambdahat_cross_e3prov1_prmp3 = -e3prov2*lambdahat3_prmp3 + e3prov3*lambdahat2_prmp3;
    double e31_prmp3 = e3_leq_bound*(lambdahat1*lambdahat_dot_e3prov_prmp3*(1 - cos_0_1_deg) + lambdahat1_prmp3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmp3*sin_0_1_deg);
    double e32_prmp3 = e3_leq_bound*(lambdahat2*lambdahat_dot_e3prov_prmp3*(1 - cos_0_1_deg) + lambdahat2_prmp3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmp3*sin_0_1_deg);
    double e33_prmp3 = e3_leq_bound*(lambdahat3*lambdahat_dot_e3prov_prmp3*(1 - cos_0_1_deg) + lambdahat3_prmp3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmp3*sin_0_1_deg);
    double xi3_prmp3 = e31_prmp3*n2 - e32_prmp3*n1;
    double xi2_prmp3 = -e31_prmp3*n3 + e33_prmp3*n1;
    double xi1_prmp3 = e32_prmp3*n3 - e33_prmp3*n2;
    double v3_prmp3 = n1*xi2_prmp3 - n2*xi1_prmp3;
    double v2_prmp3 = -n1*xi3_prmp3 + n3*xi1_prmp3;
    double v1_prmp3 = n2*xi3_prmp3 - n3*xi2_prmp3;
    double costheta_prmp3 = e31_prmp3*n1 + e32_prmp3*n2 + e33_prmp3*n3;
    double sin2theta_prmp3 = -2*costheta*costheta_prmp3;
    double xisq_prmp3 = sin2theta_prmp3;
    double Sigma_prmp3 = 2*pow(a, 2)*costheta*costheta_prmp3;
    ...
    double Deltarprime_prmp3 = Deltat_prmp3*Dinvprime + Deltatprime_prmp3*Dinv;
    double Lambt_prmp3 = (Deltat*pow(a, 2)*sin2theta_prmp3 + Deltat_prmp3*pow(a, 2)*sin2theta)*(((Deltat*pow(a, 2)*sin2theta - pow(w2, 2)) > 0) - ((Deltat*pow(a, 2)*sin2theta - pow(w2, 2)) < 0));
    ...
    double csiprime_prmp3 = (1.0/2.0)*(Deltar*Deltatprime_prmp3 + Deltar_prmp3*Deltatprime + Deltarprime*Deltat_prmp3 + Deltarprime_prmp3*Deltat)/(w2*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmp3 + (1.0/2.0)*Deltar_prmp3*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmp3 - 1.0/2.0*Deltar_prmp3*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double pdotxir_prmp3 = r*(phat1*xi1_prmp3 + phat1_prmp3*xi1 + phat2*xi2_prmp3 + phat2_prmp3*xi2 + phat3*xi3_prmp3 + phat3_prmp3*xi3);
    double pdotn_prmp3 = n1*phat1_prmp3 + n2*phat2_prmp3 + n3*phat3_prmp3;
    double pdotvr_prmp3 = r*(phat1*v1_prmp3 + phat1_prmp3*v1 + phat2*v2_prmp3 + phat2_prmp3*v2 + phat3*v3_prmp3 + phat3_prmp3*v3);
    double pphi_prmp3 = pdotxir_prmp3;
    double Qcoeff2_prmp3 = -sin2theta_prmp3/(Sigma*pow(sin2theta, 2)) - Sigma_prmp3/(pow(Sigma, 2)*sin2theta);
    double Qcoeff1_prmp3 = -Sigma*sin2theta_prmp3/(Lambt*pow(sin2theta, 2)) + Sigma_prmp3/(Lambt*sin2theta) - Lambt_prmp3*Sigma/(pow(Lambt, 2)*sin2theta);
    double DrSipn2_prmp3 = 2*Deltar*pdotn*pdotn_prmp3/Sigma - Deltar*Sigma_prmp3*pow(pdotn, 2)/pow(Sigma, 2) + Deltar_prmp3*pow(pdotn, 2)/Sigma;
    double Q_prmp3 = DrSipn2_prmp3 + 2*Qcoeff1*pdotxir*pdotxir_prmp3 + Qcoeff1_prmp3*pow(pdotxir, 2) + 2*Qcoeff2*pdotvr*pdotvr_prmp3 + Qcoeff2_prmp3*pow(pdotvr, 2);
    double Qminus1_prmp3 = Q_prmp3;
    double Jtilde_prmp3 = (1.0/2.0)*Deltar_prmp3/sqrt(Deltar);
    double exp2mu_prmp3 = Sigma_prmp3;
    double expmu_prmp3 = (1.0/2.0)*exp2mu_prmp3/sqrt(exp2mu);
    ...
    double exp2nu_prmp3 = Deltat*Sigma_prmp3/Lambt - Deltat*Lambt_prmp3*Sigma/pow(Lambt, 2) + Deltat_prmp3*Sigma/Lambt;
    double expnu_prmp3 = (1.0/2.0)*exp2nu_prmp3/sqrt(exp2nu);
    double omega_prmp3 = -Lambt_prmp3*omegatilde/pow(Lambt, 2);
    double Lambtprime_prmp3 = -Deltatprime*pow(a, 2)*sin2theta_prmp3 - Deltatprime_prmp3*pow(a, 2)*sin2theta;
    double mucostheta_prmp3 = pow(a, 2)*costheta_prmp3/Sigma - Sigma_prmp3*pow(a, 2)*costheta/pow(Sigma, 2);
    double nucostheta_prmp3 = -Deltat_prmp3*pow(a, 2)*costheta*w2/(Lambt*Sigma) + pow(a, 2)*costheta_prmp3*w2*(-Deltat + w2)/(Lambt*Sigma) - Sigma_prmp3*pow(a, 2)*costheta*w2*(-Deltat + w2)/(Lambt*pow(Sigma, 2)) - Lambt_prmp3*pow(a, 2)*costheta*w2*(-Deltat + w2)/(pow(Lambt, 2)*Sigma);
    double omegacostheta_prmp3 = -2*Deltat*pow(a, 2)*costheta_prmp3*omegatilde/pow(Lambt, 2) + 4*Deltat*Lambt_prmp3*pow(a, 2)*costheta*omegatilde/pow(Lambt, 3) - 2*Deltat_prmp3*pow(a, 2)*costheta*omegatilde/pow(Lambt, 2);
    double mur_prmp3 = -Sigma_prmp3*r/pow(Sigma, 2) + (1.0/2.0)*Deltar_prmp3/pow(Deltar, 3.0/2.0);
    double nur_prmp3 = -Sigma_prmp3*r/pow(Sigma, 2) + (1.0/2.0)*w2*(-4*Deltat_prmp3*r + Deltatprime_prmp3*w2)/(Deltat*Lambt) - 1.0/2.0*Lambt_prmp3*w2*(-4*Deltat*r + Deltatprime*w2)/(Deltat*pow(Lambt, 2)) - 1.0/2.0*Deltat_prmp3*w2*(-4*Deltat*r + Deltatprime*w2)/(pow(Deltat, 2)*Lambt);
    ...
    double sigmacoeffTerm2_prmp3 = eta*(eta*r*(-6.125*DrSipn2_prmp3*pow(u, 2) + 1.4166666666666701*Qminus1_prmp3*pow(u, 2) + r*(11.25*DrSipn2*DrSipn2_prmp3*pow(u, 2) - 1.625*DrSipn2*Qminus1_prmp3*pow(u, 2) - 1.625*DrSipn2_prmp3*Qminus1*pow(u, 2))) + r*(-0.66666666666666696*DrSipn2_prmp3*pow(u, 2) - 3.0277777777777799*Qminus1_prmp3*pow(u, 2) + r*(0.25*DrSipn2*Qminus1_prmp3*pow(u, 2) + 0.25*DrSipn2_prmp3*Qminus1*pow(u, 2) - 0.625*Qminus1*Qminus1_prmp3*pow(u, 2))));
    ...
    double Sdote3_prmp3 = S1*e31_prmp3 + S1_prmp3*e31 + S2*e32_prmp3 + S2_prmp3*e32 + S3*e33_prmp3 + S3_prmp3*e33;
    double Sdotn_prmp3 = S1_prmp3*n1 + S2_prmp3*n2 + S3_prmp3*n3;
    double Sdotv_prmp3 = S1*v1_prmp3 + S1_prmp3*v1 + S2*v2_prmp3 + S2_prmp3*v2 + S3*v3_prmp3 + S3_prmp3*v3;
    double Sdotxi_prmp3 = S1*xi1_prmp3 + S1_prmp3*xi1 + S2*xi2_prmp3 + S2_prmp3*xi2 + S3*xi3_prmp3 + S3_prmp3*xi3;
    ...
    double gammappsum_prmp3 = 2*Deltar*pdotn*pdotn_prmp3/Sigma - Deltar*Sigma_prmp3*pow(pdotn, 2)/pow(Sigma, 2) + Deltar_prmp3*pow(pdotn, 2)/Sigma - pow(pdotvr, 2)*sin2theta_prmp3/(Sigma*pow(sin2theta, 2)) + 2*pdotvr*pdotvr_prmp3/(Sigma*sin2theta) - Sigma_prmp3*pow(pdotvr, 2)/(pow(Sigma, 2)*sin2theta) - Sigma*pow(pdotxir, 2)*sin2theta_prmp3/(Lambt*pow(sin2theta, 2)) + 2*Sigma*pdotxir*pdotxir_prmp3/(Lambt*sin2theta) + Sigma_prmp3*pow(pdotxir, 2)/(Lambt*sin2theta) - Lambt_prmp3*Sigma*pow(pdotxir, 2)/(pow(Lambt, 2)*sin2theta);
    double Hnsradicand_prmp3 = Q4_prmp3 + gammappsum_prmp3;
    double alpha_prmp3 = Lambt*sqrt(Deltat*Sigma/Lambt)*((1.0/2.0)*Deltat*Sigma_prmp3/Lambt - 1.0/2.0*Deltat*Lambt_prmp3*Sigma/pow(Lambt, 2) + (1.0/2.0)*Deltat_prmp3*Sigma/Lambt)/(Deltat*Sigma);
    double betapsum_prmp3 = omegatilde*pphi_prmp3/Lambt - Lambt_prmp3*omegatilde*pphi/pow(Lambt, 2);
    double HssTerm3_prmp3 = pow(Btilde, 2)*(Sdotn*exp2mu*xisq*(-Q_prmp3 - 1.0/2.0*Q_prmp3/sqrt(Q)) + Sdotn*exp2mu*xisq_prmp3*(-sqrt(Q) - Q) + Sdotn*exp2mu_prmp3*xisq*(-sqrt(Q) - Q) + Sdotn_prmp3*exp2mu*xisq*(-sqrt(Q) - Q) + pdotvr*(-Jtilde*Sdotv*pdotn_prmp3 - Jtilde*Sdotv_prmp3*pdotn - Jtilde_prmp3*Sdotv*pdotn + Sdotn*pdotvr_prmp3 + Sdotn_prmp3*pdotvr) + pdotvr_prmp3*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr)) + Btilde*Btilde_prmp3*(-2*Sdotn*exp2mu*xisq*(sqrt(Q) + Q) + 2*pdotvr*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr)) + expmu*expnu*pdotxir*(Btilde*Jtilde*Sdotxi*pdotn_prmp3 + Btilde*Jtilde*Sdotxi_prmp3*pdotn + Btilde*Jtilde_prmp3*Sdotxi*pdotn + Btilde_prmp3*Jtilde*Sdotxi*pdotn - Sdotn*expmu*expnu*pdotxir_prmp3 - Sdotn*expmu*expnu_prmp3*pdotxir - Sdotn*expmu_prmp3*expnu*pdotxir - Sdotn_prmp3*expmu*expnu*pdotxir) + expmu*expnu*pdotxir_prmp3*(Btilde*Jtilde*Sdotxi*pdotn - Sdotn*expmu*expnu*pdotxir) + expmu*expnu_prmp3*pdotxir*(Btilde*Jtilde*Sdotxi*pdotn - Sdotn*expmu*expnu*pdotxir) + expmu_prmp3*expnu*pdotxir*(Btilde*Jtilde*Sdotxi*pdotn - Sdotn*expmu*expnu*pdotxir);
    double HssTerm3coeff_prmp3 = (1.0/2.0)*omegacostheta*(-Q_prmp3 - 1.0/2.0*Q_prmp3/sqrt(Q))/(Btilde*exp2mu*expmu*expnu*pow(sqrt(Q) + Q, 2)) + omegacostheta_prmp3/(Btilde*exp2mu*expmu*expnu*(2*sqrt(Q) + 2*Q)) - expnu_prmp3*omegacostheta/(Btilde*exp2mu*expmu*pow(expnu, 2)*(2*sqrt(Q) + 2*Q)) - expmu_prmp3*omegacostheta/(Btilde*exp2mu*pow(expmu, 2)*expnu*(2*sqrt(Q) + 2*Q)) - exp2mu_prmp3*omegacostheta/(Btilde*pow(exp2mu, 2)*expmu*expnu*(2*sqrt(Q) + 2*Q)) - Btilde_prmp3*omegacostheta/(pow(Btilde, 2)*exp2mu*expmu*expnu*(2*sqrt(Q) + 2*Q));
    double HssTerm2_prmp3 = pow(Btilde, 2)*xisq*(Jtilde*pdotn*(-Jtilde*Sdotv*pdotn_prmp3 - Jtilde*Sdotv_prmp3*pdotn - Jtilde_prmp3*Sdotv*pdotn + Sdotn*pdotvr_prmp3 + Sdotn_prmp3*pdotvr) + Jtilde*pdotn_prmp3*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr) + Jtilde_prmp3*pdotn*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr) + Sdotv*exp2mu*(Q_prmp3 + (1.0/2.0)*Q_prmp3/sqrt(Q)) + Sdotv*exp2mu_prmp3*(sqrt(Q) + Q) + Sdotv_prmp3*exp2mu*(sqrt(Q) + Q)) + pow(Btilde, 2)*xisq_prmp3*(Jtilde*pdotn*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr) + Sdotv*exp2mu*(sqrt(Q) + Q)) + Btilde*Btilde_prmp3*xisq*(2*Jtilde*pdotn*(-Jtilde*Sdotv*pdotn + Sdotn*pdotvr) + 2*Sdotv*exp2mu*(sqrt(Q) + Q)) + expmu*pdotxir*(-Btilde*Sdotxi*expnu*pdotvr_prmp3 - Btilde*Sdotxi*expnu_prmp3*pdotvr - Btilde*Sdotxi_prmp3*expnu*pdotvr - Btilde_prmp3*Sdotxi*expnu*pdotvr + Sdotv*exp2nu*expmu*pdotxir_prmp3 + Sdotv*exp2nu*expmu_prmp3*pdotxir + Sdotv*exp2nu_prmp3*expmu*pdotxir + Sdotv_prmp3*exp2nu*expmu*pdotxir) + expmu*pdotxir_prmp3*(-Btilde*Sdotxi*expnu*pdotvr + Sdotv*exp2nu*expmu*pdotxir) + expmu_prmp3*pdotxir*(-Btilde*Sdotxi*expnu*pdotvr + Sdotv*exp2nu*expmu*pdotxir);
    double HssTerm2coeff_prmp3 = (1.0/2.0)*Jtilde*omegar*(-Q_prmp3 - 1.0/2.0*Q_prmp3/sqrt(Q))/(Btilde*exp2mu*expmu*expnu*xisq*pow(sqrt(Q) + Q, 2)) - Jtilde*omegar*xisq_prmp3/(Btilde*exp2mu*expmu*expnu*pow(xisq, 2)*(2*sqrt(Q) + 2*Q)) + Jtilde*omegar_prmp3/(Btilde*exp2mu*expmu*expnu*xisq*(2*sqrt(Q) + 2*Q)) - Jtilde*expnu_prmp3*omegar/(Btilde*exp2mu*expmu*pow(expnu, 2)*xisq*(2*sqrt(Q) + 2*Q)) - Jtilde*expmu_prmp3*omegar/(Btilde*exp2mu*pow(expmu, 2)*expnu*xisq*(2*sqrt(Q) + 2*Q)) - Jtilde*exp2mu_prmp3*omegar/(Btilde*pow(exp2mu, 2)*expmu*expnu*xisq*(2*sqrt(Q) + 2*Q)) + Jtilde_prmp3*omegar/(Btilde*exp2mu*expmu*expnu*xisq*(2*sqrt(Q) + 2*Q)) - Btilde_prmp3*Jtilde*omegar/(pow(Btilde, 2)*exp2mu*expmu*expnu*xisq*(2*sqrt(Q) + 2*Q));
    double HssTerm1_prmp3 = Sdote3*omega_prmp3 + Sdote3_prmp3*omega;
    ...
    double HsoTerm2c_prmp3 = Brtilde*Jtilde*Sdotv*expmu*expnu*pdotxir_prmp3*(sqrt(Q) + 1) + Brtilde*Jtilde*Sdotv*expmu*expnu_prmp3*pdotxir*(sqrt(Q) + 1) + Brtilde*Jtilde*Sdotv*expmu_prmp3*expnu*pdotxir*(sqrt(Q) + 1) + Brtilde*Jtilde*Sdotv_prmp3*expmu*expnu*pdotxir*(sqrt(Q) + 1) + (1.0/2.0)*Brtilde*Jtilde*Q_prmp3*Sdotv*expmu*expnu*pdotxir/sqrt(Q) + Brtilde*Jtilde_prmp3*Sdotv*expmu*expnu*pdotxir*(sqrt(Q) + 1) + Brtilde_prmp3*Jtilde*Sdotv*expmu*expnu*pdotxir*(sqrt(Q) + 1);
    double HsoTerm2b_prmp3 = Btilde*expmu*expnu*pdotxir*(2*sqrt(Q) + 1)*(Jtilde*Sdotv*nur_prmp3 + Jtilde*Sdotv_prmp3*nur + Jtilde_prmp3*Sdotv*nur - Sdotn*nucostheta*xisq_prmp3 - Sdotn*nucostheta_prmp3*xisq - Sdotn_prmp3*nucostheta*xisq) + Btilde*expmu*expnu*pdotxir_prmp3*(2*sqrt(Q) + 1)*(Jtilde*Sdotv*nur - Sdotn*nucostheta*xisq) + Btilde*expmu*expnu_prmp3*pdotxir*(2*sqrt(Q) + 1)*(Jtilde*Sdotv*nur - Sdotn*nucostheta*xisq) + Btilde*expmu_prmp3*expnu*pdotxir*(2*sqrt(Q) + 1)*(Jtilde*Sdotv*nur - Sdotn*nucostheta*xisq) + Btilde*Q_prmp3*expmu*expnu*pdotxir*(Jtilde*Sdotv*nur - Sdotn*nucostheta*xisq)/sqrt(Q) + Btilde_prmp3*expmu*expnu*pdotxir*(2*sqrt(Q) + 1)*(Jtilde*Sdotv*nur - Sdotn*nucostheta*xisq);
    double HsoTerm2a_prmp3 = pow(Btilde, 2)*Jtilde*Sdotxi*(-sqrt(Q)*(nur*pdotvr_prmp3 + nur_prmp3*pdotvr + pdotn*xisq*(mucostheta_prmp3 - nucostheta_prmp3) + pdotn*xisq_prmp3*(mucostheta - nucostheta) + pdotn_prmp3*xisq*(mucostheta - nucostheta)) - mucostheta*pdotn*xisq_prmp3 - mucostheta*pdotn_prmp3*xisq - mucostheta_prmp3*pdotn*xisq + mur*pdotvr_prmp3*(sqrt(Q) + 1) + mur_prmp3*pdotvr*(sqrt(Q) + 1) + (1.0/2.0)*Q_prmp3*mur*pdotvr/sqrt(Q) + (1.0/2.0)*Q_prmp3*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta))/sqrt(Q)) + pow(Btilde, 2)*Jtilde*Sdotxi_prmp3*(sqrt(Q)*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta)) - mucostheta*pdotn*xisq + mur*pdotvr*(sqrt(Q) + 1)) + pow(Btilde, 2)*Jtilde_prmp3*Sdotxi*(sqrt(Q)*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta)) - mucostheta*pdotn*xisq + mur*pdotvr*(sqrt(Q) + 1)) + Btilde*Btilde_prmp3*Jtilde*Sdotxi*(2*sqrt(Q)*(-nur*pdotvr - pdotn*xisq*(mucostheta - nucostheta)) - 2*mucostheta*pdotn*xisq + 2*mur*pdotvr*(sqrt(Q) + 1));
    double HsoTerm2_prmp3 = HsoTerm2a_prmp3 + HsoTerm2b_prmp3 - HsoTerm2c_prmp3;
    double HsoTerm2coeff_prmp3 = expnu*(-Q_prmp3 - 1.0/2.0*Q_prmp3/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmp3/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmp3/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmp3*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmp3*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmp3 = Sdote3*exp2nu*pdotxir*(-Btilde_prmp3 + expmu*expnu_prmp3 + expmu_prmp3*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmp3*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmp3*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmp3*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmp3*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmp3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmp3*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmp3*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_S1x_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double sigmastar1_prmS1x = m2/(pow(M, 2)*m1);
    double sigma1_prmS1x = pow(M, -2);
    ...
    double condition_e3prov_lhs_prmS1x = a_prmS1x;
    double e3prov_gt_bound_prmS1x = -condition_e3prov_lhs_prmS1x*((1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs + (1.0/2.0)*fabs(condition_e3prov_lhs - condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS1x*(((condition_e3prov_lhs - condition_e3prov_rhs) > 0) - ((condition_e3prov_lhs - condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS1x)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov_leq_bound_prmS1x = -condition_e3prov_lhs_prmS1x*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS1x*(((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) > 0) - ((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS1x)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov1_prmS1x = Skerrhat1*e3prov_gt_bound_prmS1x + Skerrhat1_prmS1x*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS1x;
    double e3prov2_prmS1x = Skerrhat2*e3prov_gt_bound_prmS1x + Skerrhat2_prmS1x*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS1x;
    double e3prov3_prmS1x = Skerrhat3*e3prov_gt_bound_prmS1x + Skerrhat3_prmS1x*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS1x;
    double lambdahat_dot_e3prov_prmS1x = e3prov1_prmS1x*lambdahat1 + e3prov2_prmS1x*lambdahat2 + e3prov3_prmS1x*lambdahat3;
    double lambdahat_cross_e3prov3_prmS1x = -e3prov1_prmS1x*lambdahat2 + e3prov2_prmS1x*lambdahat1;
    double lambdahat_cross_e3prov2_prmS1x = e3prov1_prmS1x*lambdahat3 - e3prov3_prmS1x*lambdahat1;
    double lambdahat_cross_e3prov1_prmS1x = -e3prov2_prmS1x*lambdahat3 + e3prov3_prmS1x*lambdahat2;
    double e3prov_dot_n_prmS1x = e3prov1_prmS1x*n1 + e3prov2_prmS1x*n2 + e3prov3_prmS1x*n3;
    double condition_e3_lhs_prmS1x = -e3prov_dot_n_prmS1x*(((e3prov_dot_n) > 0) - ((e3prov_dot_n) < 0));
    double e3_gt_bound_prmS1x = -condition_e3_lhs_prmS1x*((1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs + (1.0/2.0)*fabs(condition_e3_lhs - condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS1x*(((condition_e3_lhs - condition_e3_rhs) > 0) - ((condition_e3_lhs - condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS1x)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e3_leq_bound_prmS1x = -condition_e3_lhs_prmS1x*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3_lhs + condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS1x*(((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) > 0) - ((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS1x)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e31_prmS1x = e3_gt_bound*e3prov1_prmS1x + e3_gt_bound_prmS1x*e3prov1 + e3_leq_bound*(cos_0_1_deg*e3prov1_prmS1x + lambdahat1*lambdahat_dot_e3prov_prmS1x*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmS1x*sin_0_1_deg) + e3_leq_bound_prmS1x*(cos_0_1_deg*e3prov1 + lambdahat1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1*sin_0_1_deg);
    double e32_prmS1x = e3_gt_bound*e3prov2_prmS1x + e3_gt_bound_prmS1x*e3prov2 + e3_leq_bound*(cos_0_1_deg*e3prov2_prmS1x + lambdahat2*lambdahat_dot_e3prov_prmS1x*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmS1x*sin_0_1_deg) + e3_leq_bound_prmS1x*(cos_0_1_deg*e3prov2 + lambdahat2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2*sin_0_1_deg);
    double e33_prmS1x = e3_gt_bound*e3prov3_prmS1x + e3_gt_bound_prmS1x*e3prov3 + e3_leq_bound*(cos_0_1_deg*e3prov3_prmS1x + lambdahat3*lambdahat_dot_e3prov_prmS1x*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmS1x*sin_0_1_deg) + e3_leq_bound_prmS1x*(cos_0_1_deg*e3prov3 + lambdahat3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3*sin_0_1_deg);
    ...
    double Deltarprime_prmS1x = Deltat_prmS1x*Dinvprime + Deltatprime_prmS1x*Dinv;
    ...
    double csiprime_prmS1x = 4.0*r*w2_prmS1x*sqrt(Deltar*Deltat)/pow(w2, 3) + (1.0/2.0)*(Deltar*Deltatprime_prmS1x + Deltar_prmS1x*Deltatprime + Deltarprime*Deltat_prmS1x + Deltarprime_prmS1x*Deltat)/(w2*sqrt(Deltar*Deltat)) - 1.0/2.0*w2_prmS1x*(Deltar*Deltatprime + Deltarprime*Deltat)/(pow(w2, 2)*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmS1x + (1.0/2.0)*Deltar_prmS1x*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmS1x - 1.0/2.0*Deltar_prmS1x*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double sigmacoeffTerm2_prmS1x = eta*(eta*r*(-6.125*DrSipn2_prmS1x*pow(u, 2) + 1.4166666666666701*Qminus1_prmS1x*pow(u, 2) + r*(11.25*DrSipn2*DrSipn2_prmS1x*pow(u, 2) - 1.625*DrSipn2*Qminus1_prmS1x*pow(u, 2) - 1.625*DrSipn2_prmS1x*Qminus1*pow(u, 2))) + r*(-0.66666666666666696*DrSipn2_prmS1x*pow(u, 2) - 3.0277777777777799*Qminus1_prmS1x*pow(u, 2) + r*(0.25*DrSipn2*Qminus1_prmS1x*pow(u, 2) + 0.25*DrSipn2_prmS1x*Qminus1*pow(u, 2) - 0.625*Qminus1*Qminus1_prmS1x*pow(u, 2))));
    ...
    double Sdote3_prmS1x = S1*e31_prmS1x + S1_prmS1x*e31 + S2*e32_prmS1x + S2_prmS1x*e32 + S3*e33_prmS1x + S3_prmS1x*e33;
    ...
    double HssTerm1_prmS1x = Sdote3*omega_prmS1x + Sdote3_prmS1x*omega;
    ...
    double HsoTerm2coeff_prmS1x = expnu*(-Q_prmS1x - 1.0/2.0*Q_prmS1x/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmS1x/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmS1x/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmS1x*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmS1x*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmS1x = Sdote3*exp2nu*pdotxir*(-Btilde_prmS1x + expmu*expnu_prmS1x + expmu_prmS1x*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmS1x*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmS1x*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmS1x*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmS1x*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmS1x*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmS1x*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmS1x*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_S1y_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double sigmastar2_prmS1y = m2/(pow(M, 2)*m1);
    double sigma2_prmS1y = pow(M, -2);
    ...
    double condition_e3prov_lhs_prmS1y = a_prmS1y;
    double e3prov_gt_bound_prmS1y = -condition_e3prov_lhs_prmS1y*((1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs + (1.0/2.0)*fabs(condition_e3prov_lhs - condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS1y*(((condition_e3prov_lhs - condition_e3prov_rhs) > 0) - ((condition_e3prov_lhs - condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS1y)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov_leq_bound_prmS1y = -condition_e3prov_lhs_prmS1y*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS1y*(((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) > 0) - ((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS1y)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov1_prmS1y = Skerrhat1*e3prov_gt_bound_prmS1y + Skerrhat1_prmS1y*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS1y;
    double e3prov2_prmS1y = Skerrhat2*e3prov_gt_bound_prmS1y + Skerrhat2_prmS1y*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS1y;
    double e3prov3_prmS1y = Skerrhat3*e3prov_gt_bound_prmS1y + Skerrhat3_prmS1y*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS1y;
    double lambdahat_dot_e3prov_prmS1y = e3prov1_prmS1y*lambdahat1 + e3prov2_prmS1y*lambdahat2 + e3prov3_prmS1y*lambdahat3;
    double lambdahat_cross_e3prov3_prmS1y = -e3prov1_prmS1y*lambdahat2 + e3prov2_prmS1y*lambdahat1;
    double lambdahat_cross_e3prov2_prmS1y = e3prov1_prmS1y*lambdahat3 - e3prov3_prmS1y*lambdahat1;
    double lambdahat_cross_e3prov1_prmS1y = -e3prov2_prmS1y*lambdahat3 + e3prov3_prmS1y*lambdahat2;
    double e3prov_dot_n_prmS1y = e3prov1_prmS1y*n1 + e3prov2_prmS1y*n2 + e3prov3_prmS1y*n3;
    double condition_e3_lhs_prmS1y = -e3prov_dot_n_prmS1y*(((e3prov_dot_n) > 0) - ((e3prov_dot_n) < 0));
    double e3_gt_bound_prmS1y = -condition_e3_lhs_prmS1y*((1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs + (1.0/2.0)*fabs(condition_e3_lhs - condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS1y*(((condition_e3_lhs - condition_e3_rhs) > 0) - ((condition_e3_lhs - condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS1y)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e3_leq_bound_prmS1y = -condition_e3_lhs_prmS1y*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3_lhs + condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS1y*(((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) > 0) - ((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS1y)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e31_prmS1y = e3_gt_bound*e3prov1_prmS1y + e3_gt_bound_prmS1y*e3prov1 + e3_leq_bound*(cos_0_1_deg*e3prov1_prmS1y + lambdahat1*lambdahat_dot_e3prov_prmS1y*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmS1y*sin_0_1_deg) + e3_leq_bound_prmS1y*(cos_0_1_deg*e3prov1 + lambdahat1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1*sin_0_1_deg);
    double e32_prmS1y = e3_gt_bound*e3prov2_prmS1y + e3_gt_bound_prmS1y*e3prov2 + e3_leq_bound*(cos_0_1_deg*e3prov2_prmS1y + lambdahat2*lambdahat_dot_e3prov_prmS1y*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmS1y*sin_0_1_deg) + e3_leq_bound_prmS1y*(cos_0_1_deg*e3prov2 + lambdahat2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2*sin_0_1_deg);
    double e33_prmS1y = e3_gt_bound*e3prov3_prmS1y + e3_gt_bound_prmS1y*e3prov3 + e3_leq_bound*(cos_0_1_deg*e3prov3_prmS1y + lambdahat3*lambdahat_dot_e3prov_prmS1y*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmS1y*sin_0_1_deg) + e3_leq_bound_prmS1y*(cos_0_1_deg*e3prov3 + lambdahat3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3*sin_0_1_deg);
    ...
    double Deltarprime_prmS1y = Deltat_prmS1y*Dinvprime + Deltatprime_prmS1y*Dinv;
    ...
    double csiprime_prmS1y = 4.0*r*w2_prmS1y*sqrt(Deltar*Deltat)/pow(w2, 3) + (1.0/2.0)*(Deltar*Deltatprime_prmS1y + Deltar_prmS1y*Deltatprime + Deltarprime*Deltat_prmS1y + Deltarprime_prmS1y*Deltat)/(w2*sqrt(Deltar*Deltat)) - 1.0/2.0*w2_prmS1y*(Deltar*Deltatprime + Deltarprime*Deltat)/(pow(w2, 2)*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmS1y + (1.0/2.0)*Deltar_prmS1y*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmS1y - 1.0/2.0*Deltar_prmS1y*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double sigmacoeffTerm2_prmS1y = eta*(eta*r*(-6.125*DrSipn2_prmS1y*pow(u, 2) + 1.4166666666666701*Qminus1_prmS1y*pow(u, 2) + r*(11.25*DrSipn2*DrSipn2_prmS1y*pow(u, 2) - 1.625*DrSipn2*Qminus1_prmS1y*pow(u, 2) - 1.625*DrSipn2_prmS1y*Qminus1*pow(u, 2))) + r*(-0.66666666666666696*DrSipn2_prmS1y*pow(u, 2) - 3.0277777777777799*Qminus1_prmS1y*pow(u, 2) + r*(0.25*DrSipn2*Qminus1_prmS1y*pow(u, 2) + 0.25*DrSipn2_prmS1y*Qminus1*pow(u, 2) - 0.625*Qminus1*Qminus1_prmS1y*pow(u, 2))));
    ...
    double Sdote3_prmS1y = S1*e31_prmS1y + S1_prmS1y*e31 + S2*e32_prmS1y + S2_prmS1y*e32 + S3*e33_prmS1y + S3_prmS1y*e33;
    ...
    double HssTerm1_prmS1y = Sdote3*omega_prmS1y + Sdote3_prmS1y*omega;
    ...
    double HsoTerm2coeff_prmS1y = expnu*(-Q_prmS1y - 1.0/2.0*Q_prmS1y/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmS1y/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmS1y/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmS1y*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmS1y*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmS1y = Sdote3*exp2nu*pdotxir*(-Btilde_prmS1y + expmu*expnu_prmS1y + expmu_prmS1y*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmS1y*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmS1y*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmS1y*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmS1y*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmS1y*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmS1y*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmS1y*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_S1z_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double sigmastar3_prmS1z = m2/(pow(M, 2)*m1);
    double sigma3_prmS1z = pow(M, -2);
    ...
    double condition_e3prov_lhs_prmS1z = a_prmS1z;
    double e3prov_gt_bound_prmS1z = -condition_e3prov_lhs_prmS1z*((1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs + (1.0/2.0)*fabs(condition_e3prov_lhs - condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS1z*(((condition_e3prov_lhs - condition_e3prov_rhs) > 0) - ((condition_e3prov_lhs - condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS1z)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov_leq_bound_prmS1z = -condition_e3prov_lhs_prmS1z*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS1z*(((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) > 0) - ((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS1z)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov1_prmS1z = Skerrhat1*e3prov_gt_bound_prmS1z + Skerrhat1_prmS1z*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS1z;
    double e3prov2_prmS1z = Skerrhat2*e3prov_gt_bound_prmS1z + Skerrhat2_prmS1z*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS1z;
    double e3prov3_prmS1z = Skerrhat3*e3prov_gt_bound_prmS1z + Skerrhat3_prmS1z*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS1z;
    double lambdahat_dot_e3prov_prmS1z = e3prov1_prmS1z*lambdahat1 + e3prov2_prmS1z*lambdahat2 + e3prov3_prmS1z*lambdahat3;
    double lambdahat_cross_e3prov3_prmS1z = -e3prov1_prmS1z*lambdahat2 + e3prov2_prmS1z*lambdahat1;
    double lambdahat_cross_e3prov2_prmS1z = e3prov1_prmS1z*lambdahat3 - e3prov3_prmS1z*lambdahat1;
    double lambdahat_cross_e3prov1_prmS1z = -e3prov2_prmS1z*lambdahat3 + e3prov3_prmS1z*lambdahat2;
    double e3prov_dot_n_prmS1z = e3prov1_prmS1z*n1 + e3prov2_prmS1z*n2 + e3prov3_prmS1z*n3;
    double condition_e3_lhs_prmS1z = -e3prov_dot_n_prmS1z*(((e3prov_dot_n) > 0) - ((e3prov_dot_n) < 0));
    double e3_gt_bound_prmS1z = -condition_e3_lhs_prmS1z*((1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs + (1.0/2.0)*fabs(condition_e3_lhs - condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS1z*(((condition_e3_lhs - condition_e3_rhs) > 0) - ((condition_e3_lhs - condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS1z)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e3_leq_bound_prmS1z = -condition_e3_lhs_prmS1z*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3_lhs + condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS1z*(((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) > 0) - ((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS1z)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e31_prmS1z = e3_gt_bound*e3prov1_prmS1z + e3_gt_bound_prmS1z*e3prov1 + e3_leq_bound*(cos_0_1_deg*e3prov1_prmS1z + lambdahat1*lambdahat_dot_e3prov_prmS1z*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmS1z*sin_0_1_deg) + e3_leq_bound_prmS1z*(cos_0_1_deg*e3prov1 + lambdahat1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1*sin_0_1_deg);
    double e32_prmS1z = e3_gt_bound*e3prov2_prmS1z + e3_gt_bound_prmS1z*e3prov2 + e3_leq_bound*(cos_0_1_deg*e3prov2_prmS1z + lambdahat2*lambdahat_dot_e3prov_prmS1z*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmS1z*sin_0_1_deg) + e3_leq_bound_prmS1z*(cos_0_1_deg*e3prov2 + lambdahat2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2*sin_0_1_deg);
    double e33_prmS1z = e3_gt_bound*e3prov3_prmS1z + e3_gt_bound_prmS1z*e3prov3 + e3_leq_bound*(cos_0_1_deg*e3prov3_prmS1z + lambdahat3*lambdahat_dot_e3prov_prmS1z*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmS1z*sin_0_1_deg) + e3_leq_bound_prmS1z*(cos_0_1_deg*e3prov3 + lambdahat3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3*sin_0_1_deg);
    ...
    double Deltarprime_prmS1z = Deltat_prmS1z*Dinvprime + Deltatprime_prmS1z*Dinv;
    ...
    double csiprime_prmS1z = 4.0*r*w2_prmS1z*sqrt(Deltar*Deltat)/pow(w2, 3) + (1.0/2.0)*(Deltar*Deltatprime_prmS1z + Deltar_prmS1z*Deltatprime + Deltarprime*Deltat_prmS1z + Deltarprime_prmS1z*Deltat)/(w2*sqrt(Deltar*Deltat)) - 1.0/2.0*w2_prmS1z*(Deltar*Deltatprime + Deltarprime*Deltat)/(pow(w2, 2)*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmS1z + (1.0/2.0)*Deltar_prmS1z*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmS1z - 1.0/2.0*Deltar_prmS1z*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double sigmacoeffTerm2_prmS1z = eta*(eta*r*(-6.125*DrSipn2_prmS1z*pow(u, 2) + 1.4166666666666701*Qminus1_prmS1z*pow(u, 2) + r*(11.25*DrSipn2*DrSipn2_prmS1z*pow(u, 2) - 1.625*DrSipn2*Qminus1_prmS1z*pow(u, 2) - 1.625*DrSipn2_prmS1z*Qminus1*pow(u, 2))) + r*(-0.66666666666666696*DrSipn2_prmS1z*pow(u, 2) - 3.0277777777777799*Qminus1_prmS1z*pow(u, 2) + r*(0.25*DrSipn2*Qminus1_prmS1z*pow(u, 2) + 0.25*DrSipn2_prmS1z*Qminus1*pow(u, 2) - 0.625*Qminus1*Qminus1_prmS1z*pow(u, 2))));
    ...
    double Sdote3_prmS1z = S1*e31_prmS1z + S1_prmS1z*e31 + S2*e32_prmS1z + S2_prmS1z*e32 + S3*e33_prmS1z + S3_prmS1z*e33;
    ...
    double HssTerm1_prmS1z = Sdote3*omega_prmS1z + Sdote3_prmS1z*omega;
    ...
    double HsoTerm2coeff_prmS1z = expnu*(-Q_prmS1z - 1.0/2.0*Q_prmS1z/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmS1z/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmS1z/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmS1z*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmS1z*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmS1z = Sdote3*exp2nu*pdotxir*(-Btilde_prmS1z + expmu*expnu_prmS1z + expmu_prmS1z*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmS1z*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmS1z*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmS1z*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmS1z*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmS1z*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmS1z*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmS1z*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_S2x_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double sigmastar1_prmS2x = m1/(pow(M, 2)*m2);
    double sigma1_prmS2x = pow(M, -2);
    ...
    double condition_e3prov_lhs_prmS2x = a_prmS2x;
    double e3prov_gt_bound_prmS2x = -condition_e3prov_lhs_prmS2x*((1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs + (1.0/2.0)*fabs(condition_e3prov_lhs - condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS2x*(((condition_e3prov_lhs - condition_e3prov_rhs) > 0) - ((condition_e3prov_lhs - condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS2x)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov_leq_bound_prmS2x = -condition_e3prov_lhs_prmS2x*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS2x*(((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) > 0) - ((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS2x)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov1_prmS2x = Skerrhat1*e3prov_gt_bound_prmS2x + Skerrhat1_prmS2x*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS2x;
    double e3prov2_prmS2x = Skerrhat2*e3prov_gt_bound_prmS2x + Skerrhat2_prmS2x*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS2x;
    double e3prov3_prmS2x = Skerrhat3*e3prov_gt_bound_prmS2x + Skerrhat3_prmS2x*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS2x;
    double lambdahat_dot_e3prov_prmS2x = e3prov1_prmS2x*lambdahat1 + e3prov2_prmS2x*lambdahat2 + e3prov3_prmS2x*lambdahat3;
    double lambdahat_cross_e3prov3_prmS2x = -e3prov1_prmS2x*lambdahat2 + e3prov2_prmS2x*lambdahat1;
    double lambdahat_cross_e3prov2_prmS2x = e3prov1_prmS2x*lambdahat3 - e3prov3_prmS2x*lambdahat1;
    double lambdahat_cross_e3prov1_prmS2x = -e3prov2_prmS2x*lambdahat3 + e3prov3_prmS2x*lambdahat2;
    double e3prov_dot_n_prmS2x = e3prov1_prmS2x*n1 + e3prov2_prmS2x*n2 + e3prov3_prmS2x*n3;
    double condition_e3_lhs_prmS2x = -e3prov_dot_n_prmS2x*(((e3prov_dot_n) > 0) - ((e3prov_dot_n) < 0));
    double e3_gt_bound_prmS2x = -condition_e3_lhs_prmS2x*((1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs + (1.0/2.0)*fabs(condition_e3_lhs - condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS2x*(((condition_e3_lhs - condition_e3_rhs) > 0) - ((condition_e3_lhs - condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS2x)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e3_leq_bound_prmS2x = -condition_e3_lhs_prmS2x*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3_lhs + condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS2x*(((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) > 0) - ((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS2x)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e31_prmS2x = e3_gt_bound*e3prov1_prmS2x + e3_gt_bound_prmS2x*e3prov1 + e3_leq_bound*(cos_0_1_deg*e3prov1_prmS2x + lambdahat1*lambdahat_dot_e3prov_prmS2x*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmS2x*sin_0_1_deg) + e3_leq_bound_prmS2x*(cos_0_1_deg*e3prov1 + lambdahat1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1*sin_0_1_deg);
    double e32_prmS2x = e3_gt_bound*e3prov2_prmS2x + e3_gt_bound_prmS2x*e3prov2 + e3_leq_bound*(cos_0_1_deg*e3prov2_prmS2x + lambdahat2*lambdahat_dot_e3prov_prmS2x*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmS2x*sin_0_1_deg) + e3_leq_bound_prmS2x*(cos_0_1_deg*e3prov2 + lambdahat2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2*sin_0_1_deg);
    double e33_prmS2x = e3_gt_bound*e3prov3_prmS2x + e3_gt_bound_prmS2x*e3prov3 + e3_leq_bound*(cos_0_1_deg*e3prov3_prmS2x + lambdahat3*lambdahat_dot_e3prov_prmS2x*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmS2x*sin_0_1_deg) + e3_leq_bound_prmS2x*(cos_0_1_deg*e3prov3 + lambdahat3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3*sin_0_1_deg);
    ...
    double Deltarprime_prmS2x = Deltat_prmS2x*Dinvprime + Deltatprime_prmS2x*Dinv;
    ...
    double csiprime_prmS2x = 4.0*r*w2_prmS2x*sqrt(Deltar*Deltat)/pow(w2, 3) + (1.0/2.0)*(Deltar*Deltatprime_prmS2x + Deltar_prmS2x*Deltatprime + Deltarprime*Deltat_prmS2x + Deltarprime_prmS2x*Deltat)/(w2*sqrt(Deltar*Deltat)) - 1.0/2.0*w2_prmS2x*(Deltar*Deltatprime + Deltarprime*Deltat)/(pow(w2, 2)*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmS2x + (1.0/2.0)*Deltar_prmS2x*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmS2x - 1.0/2.0*Deltar_prmS2x*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double sigmacoeffTerm2_prmS2x = eta*(eta*r*(-6.125*DrSipn2_prmS2x*pow(u, 2) + 1.4166666666666701*Qminus1_prmS2x*pow(u, 2) + r*(11.25*DrSipn2*DrSipn2_prmS2x*pow(u, 2) - 1.625*DrSipn2*Qminus1_prmS2x*pow(u, 2) - 1.625*DrSipn2_prmS2x*Qminus1*pow(u, 2))) + r*(-0.66666666666666696*DrSipn2_prmS2x*pow(u, 2) - 3.0277777777777799*Qminus1_prmS2x*pow(u, 2) + r*(0.25*DrSipn2*Qminus1_prmS2x*pow(u, 2) + 0.25*DrSipn2_prmS2x*Qminus1*pow(u, 2) - 0.625*Qminus1*Qminus1_prmS2x*pow(u, 2))));
    ...
    double Sdote3_prmS2x = S1*e31_prmS2x + S1_prmS2x*e31 + S2*e32_prmS2x + S2_prmS2x*e32 + S3*e33_prmS2x + S3_prmS2x*e33;
    ...
    double HssTerm1_prmS2x = Sdote3*omega_prmS2x + Sdote3_prmS2x*omega;
    ...
    double HsoTerm2coeff_prmS2x = expnu*(-Q_prmS2x - 1.0/2.0*Q_prmS2x/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmS2x/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmS2x/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmS2x*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmS2x*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmS2x = Sdote3*exp2nu*pdotxir*(-Btilde_prmS2x + expmu*expnu_prmS2x + expmu_prmS2x*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmS2x*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmS2x*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmS2x*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmS2x*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmS2x*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmS2x*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmS2x*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_S2y_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double sigmastar2_prmS2y = m1/(pow(M, 2)*m2);
    double sigma2_prmS2y = pow(M, -2);
    ...
    double condition_e3prov_lhs_prmS2y = a_prmS2y;
    double e3prov_gt_bound_prmS2y = -condition_e3prov_lhs_prmS2y*((1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs + (1.0/2.0)*fabs(condition_e3prov_lhs - condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS2y*(((condition_e3prov_lhs - condition_e3prov_rhs) > 0) - ((condition_e3prov_lhs - condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS2y)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov_leq_bound_prmS2y = -condition_e3prov_lhs_prmS2y*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS2y*(((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) > 0) - ((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS2y)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov1_prmS2y = Skerrhat1*e3prov_gt_bound_prmS2y + Skerrhat1_prmS2y*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS2y;
    double e3prov2_prmS2y = Skerrhat2*e3prov_gt_bound_prmS2y + Skerrhat2_prmS2y*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS2y;
    double e3prov3_prmS2y = Skerrhat3*e3prov_gt_bound_prmS2y + Skerrhat3_prmS2y*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS2y;
    double lambdahat_dot_e3prov_prmS2y = e3prov1_prmS2y*lambdahat1 + e3prov2_prmS2y*lambdahat2 + e3prov3_prmS2y*lambdahat3;
    double lambdahat_cross_e3prov3_prmS2y = -e3prov1_prmS2y*lambdahat2 + e3prov2_prmS2y*lambdahat1;
    double lambdahat_cross_e3prov2_prmS2y = e3prov1_prmS2y*lambdahat3 - e3prov3_prmS2y*lambdahat1;
    double lambdahat_cross_e3prov1_prmS2y = -e3prov2_prmS2y*lambdahat3 + e3prov3_prmS2y*lambdahat2;
    double e3prov_dot_n_prmS2y = e3prov1_prmS2y*n1 + e3prov2_prmS2y*n2 + e3prov3_prmS2y*n3;
    double condition_e3_lhs_prmS2y = -e3prov_dot_n_prmS2y*(((e3prov_dot_n) > 0) - ((e3prov_dot_n) < 0));
    double e3_gt_bound_prmS2y = -condition_e3_lhs_prmS2y*((1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs + (1.0/2.0)*fabs(condition_e3_lhs - condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS2y*(((condition_e3_lhs - condition_e3_rhs) > 0) - ((condition_e3_lhs - condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS2y)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e3_leq_bound_prmS2y = -condition_e3_lhs_prmS2y*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3_lhs + condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS2y*(((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) > 0) - ((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS2y)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e31_prmS2y = e3_gt_bound*e3prov1_prmS2y + e3_gt_bound_prmS2y*e3prov1 + e3_leq_bound*(cos_0_1_deg*e3prov1_prmS2y + lambdahat1*lambdahat_dot_e3prov_prmS2y*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmS2y*sin_0_1_deg) + e3_leq_bound_prmS2y*(cos_0_1_deg*e3prov1 + lambdahat1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1*sin_0_1_deg);
    double e32_prmS2y = e3_gt_bound*e3prov2_prmS2y + e3_gt_bound_prmS2y*e3prov2 + e3_leq_bound*(cos_0_1_deg*e3prov2_prmS2y + lambdahat2*lambdahat_dot_e3prov_prmS2y*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmS2y*sin_0_1_deg) + e3_leq_bound_prmS2y*(cos_0_1_deg*e3prov2 + lambdahat2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2*sin_0_1_deg);
    double e33_prmS2y = e3_gt_bound*e3prov3_prmS2y + e3_gt_bound_prmS2y*e3prov3 + e3_leq_bound*(cos_0_1_deg*e3prov3_prmS2y + lambdahat3*lambdahat_dot_e3prov_prmS2y*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmS2y*sin_0_1_deg) + e3_leq_bound_prmS2y*(cos_0_1_deg*e3prov3 + lambdahat3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3*sin_0_1_deg);
    ...
    double Deltarprime_prmS2y = Deltat_prmS2y*Dinvprime + Deltatprime_prmS2y*Dinv;
    ...
    double csiprime_prmS2y = 4.0*r*w2_prmS2y*sqrt(Deltar*Deltat)/pow(w2, 3) + (1.0/2.0)*(Deltar*Deltatprime_prmS2y + Deltar_prmS2y*Deltatprime + Deltarprime*Deltat_prmS2y + Deltarprime_prmS2y*Deltat)/(w2*sqrt(Deltar*Deltat)) - 1.0/2.0*w2_prmS2y*(Deltar*Deltatprime + Deltarprime*Deltat)/(pow(w2, 2)*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmS2y + (1.0/2.0)*Deltar_prmS2y*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmS2y - 1.0/2.0*Deltar_prmS2y*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double sigmacoeffTerm2_prmS2y = eta*(eta*r*(-6.125*DrSipn2_prmS2y*pow(u, 2) + 1.4166666666666701*Qminus1_prmS2y*pow(u, 2) + r*(11.25*DrSipn2*DrSipn2_prmS2y*pow(u, 2) - 1.625*DrSipn2*Qminus1_prmS2y*pow(u, 2) - 1.625*DrSipn2_prmS2y*Qminus1*pow(u, 2))) + r*(-0.66666666666666696*DrSipn2_prmS2y*pow(u, 2) - 3.0277777777777799*Qminus1_prmS2y*pow(u, 2) + r*(0.25*DrSipn2*Qminus1_prmS2y*pow(u, 2) + 0.25*DrSipn2_prmS2y*Qminus1*pow(u, 2) - 0.625*Qminus1*Qminus1_prmS2y*pow(u, 2))));
    ...
    double Sdote3_prmS2y = S1*e31_prmS2y + S1_prmS2y*e31 + S2*e32_prmS2y + S2_prmS2y*e32 + S3*e33_prmS2y + S3_prmS2y*e33;
    ...
    double HssTerm1_prmS2y = Sdote3*omega_prmS2y + Sdote3_prmS2y*omega;
    ...
    double HsoTerm2coeff_prmS2y = expnu*(-Q_prmS2y - 1.0/2.0*Q_prmS2y/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmS2y/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmS2y/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmS2y*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmS2y*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmS2y = Sdote3*exp2nu*pdotxir*(-Btilde_prmS2y + expmu*expnu_prmS2y + expmu_prmS2y*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmS2y*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmS2y*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmS2y*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmS2y*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmS2y*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmS2y*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmS2y*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
...
double ham_S2z_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    double EMgamma = 0.57721566490153286;
    double M = m1 + m2;
    double mu = m1*m2/M;
    double eta = mu/M;
    ...
    double sigmastar3_prmS2z = m1/(pow(M, 2)*m2);
    double sigma3_prmS2z = pow(M, -2);
    ...
    double condition_e3prov_lhs_prmS2z = a_prmS2z;
    double e3prov_gt_bound_prmS2z = -condition_e3prov_lhs_prmS2z*((1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs + (1.0/2.0)*fabs(condition_e3prov_lhs - condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS2z*(((condition_e3prov_lhs - condition_e3prov_rhs) > 0) - ((condition_e3prov_lhs - condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS2z)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov_leq_bound_prmS2z = -condition_e3prov_lhs_prmS2z*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3prov_lhs - 1.0/2.0*condition_e3prov_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs))/pow(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs, 2) + ((1.0/2.0)*condition_e3prov_lhs_prmS2z*(((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) > 0) - ((TINYDOUBLE - condition_e3prov_lhs + condition_e3prov_rhs) < 0)) + (1.0/2.0)*condition_e3prov_lhs_prmS2z)/(-TINYDOUBLE + condition_e3prov_lhs - condition_e3prov_rhs);
    double e3prov1_prmS2z = Skerrhat1*e3prov_gt_bound_prmS2z + Skerrhat1_prmS2z*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS2z;
    double e3prov2_prmS2z = Skerrhat2*e3prov_gt_bound_prmS2z + Skerrhat2_prmS2z*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS2z;
    double e3prov3_prmS2z = Skerrhat3*e3prov_gt_bound_prmS2z + Skerrhat3_prmS2z*e3prov_gt_bound + 0.57735026918962595*e3prov_leq_bound_prmS2z;
    double lambdahat_dot_e3prov_prmS2z = e3prov1_prmS2z*lambdahat1 + e3prov2_prmS2z*lambdahat2 + e3prov3_prmS2z*lambdahat3;
    double lambdahat_cross_e3prov3_prmS2z = -e3prov1_prmS2z*lambdahat2 + e3prov2_prmS2z*lambdahat1;
    double lambdahat_cross_e3prov2_prmS2z = e3prov1_prmS2z*lambdahat3 - e3prov3_prmS2z*lambdahat1;
    double lambdahat_cross_e3prov1_prmS2z = -e3prov2_prmS2z*lambdahat3 + e3prov3_prmS2z*lambdahat2;
    double e3prov_dot_n_prmS2z = e3prov1_prmS2z*n1 + e3prov2_prmS2z*n2 + e3prov3_prmS2z*n3;
    double condition_e3_lhs_prmS2z = -e3prov_dot_n_prmS2z*(((e3prov_dot_n) > 0) - ((e3prov_dot_n) < 0));
    double e3_gt_bound_prmS2z = -condition_e3_lhs_prmS2z*((1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs + (1.0/2.0)*fabs(condition_e3_lhs - condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS2z*(((condition_e3_lhs - condition_e3_rhs) > 0) - ((condition_e3_lhs - condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS2z)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e3_leq_bound_prmS2z = -condition_e3_lhs_prmS2z*(-1.0/2.0*TINYDOUBLE + (1.0/2.0)*condition_e3_lhs - 1.0/2.0*condition_e3_rhs - 1.0/2.0*fabs(TINYDOUBLE - condition_e3_lhs + condition_e3_rhs))/pow(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs, 2) + ((1.0/2.0)*condition_e3_lhs_prmS2z*(((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) > 0) - ((TINYDOUBLE - condition_e3_lhs + condition_e3_rhs) < 0)) + (1.0/2.0)*condition_e3_lhs_prmS2z)/(-TINYDOUBLE + condition_e3_lhs - condition_e3_rhs);
    double e31_prmS2z = e3_gt_bound*e3prov1_prmS2z + e3_gt_bound_prmS2z*e3prov1 + e3_leq_bound*(cos_0_1_deg*e3prov1_prmS2z + lambdahat1*lambdahat_dot_e3prov_prmS2z*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1_prmS2z*sin_0_1_deg) + e3_leq_bound_prmS2z*(cos_0_1_deg*e3prov1 + lambdahat1*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov1*sin_0_1_deg);
    double e32_prmS2z = e3_gt_bound*e3prov2_prmS2z + e3_gt_bound_prmS2z*e3prov2 + e3_leq_bound*(cos_0_1_deg*e3prov2_prmS2z + lambdahat2*lambdahat_dot_e3prov_prmS2z*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2_prmS2z*sin_0_1_deg) + e3_leq_bound_prmS2z*(cos_0_1_deg*e3prov2 + lambdahat2*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov2*sin_0_1_deg);
    double e33_prmS2z = e3_gt_bound*e3prov3_prmS2z + e3_gt_bound_prmS2z*e3prov3 + e3_leq_bound*(cos_0_1_deg*e3prov3_prmS2z + lambdahat3*lambdahat_dot_e3prov_prmS2z*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3_prmS2z*sin_0_1_deg) + e3_leq_bound_prmS2z*(cos_0_1_deg*e3prov3 + lambdahat3*lambdahat_dot_e3prov*(1 - cos_0_1_deg) + lambdahat_cross_e3prov3*sin_0_1_deg);
    ...
    double Deltarprime_prmS2z = Deltat_prmS2z*Dinvprime + Deltatprime_prmS2z*Dinv;
    ...
    double csiprime_prmS2z = 4.0*r*w2_prmS2z*sqrt(Deltar*Deltat)/pow(w2, 3) + (1.0/2.0)*(Deltar*Deltatprime_prmS2z + Deltar_prmS2z*Deltatprime + Deltarprime*Deltat_prmS2z + Deltarprime_prmS2z*Deltat)/(w2*sqrt(Deltar*Deltat)) - 1.0/2.0*w2_prmS2z*(Deltar*Deltatprime + Deltarprime*Deltat)/(pow(w2, 2)*sqrt(Deltar*Deltat)) - 2.0*r*sqrt(Deltar*Deltat)*((1.0/2.0)*Deltar*Deltat_prmS2z + (1.0/2.0)*Deltar_prmS2z*Deltat)/(Deltar*Deltat*pow(w2, 2)) + (1.0/2.0)*(-1.0/2.0*Deltar*Deltat_prmS2z - 1.0/2.0*Deltar_prmS2z*Deltat)*(Deltar*Deltatprime + Deltarprime*Deltat)/(Deltar*Deltat*w2*sqrt(Deltar*Deltat));
    ...
    double sigmacoeffTerm2_prmS2z = eta*(eta*r*(-6.125*DrSipn2_prmS2z*pow(u, 2) + 1.4166666666666701*Qminus1_prmS2z*pow(u, 2) + r*(11.25*DrSipn2*DrSipn2_prmS2z*pow(u, 2) - 1.625*DrSipn2*Qminus1_prmS2z*pow(u, 2) - 1.625*DrSipn2_prmS2z*Qminus1*pow(u, 2))) + r*(-0.66666666666666696*DrSipn2_prmS2z*pow(u, 2) - 3.0277777777777799*Qminus1_prmS2z*pow(u, 2) + r*(0.25*DrSipn2*Qminus1_prmS2z*pow(u, 2) + 0.25*DrSipn2_prmS2z*Qminus1*pow(u, 2) - 0.625*Qminus1*Qminus1_prmS2z*pow(u, 2))));
    ...
    double Sdote3_prmS2z = S1*e31_prmS2z + S1_prmS2z*e31 + S2*e32_prmS2z + S2_prmS2z*e32 + S3*e33_prmS2z + S3_prmS2z*e33;
    ...
    double HssTerm1_prmS2z = Sdote3*omega_prmS2z + Sdote3_prmS2z*omega;
    ...
    double HsoTerm2coeff_prmS2z = expnu*(-Q_prmS2z - 1.0/2.0*Q_prmS2z/sqrt(Q))/(Deltat*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmS2z/(Deltat*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmS2z/(Deltat*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmS2z*expnu/(Deltat*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - Deltat_prmS2z*expnu/(pow(Deltat, 2)*exp2mu*xisq*(sqrt(Q) + Q));
    double HsoTerm1_prmS2z = Sdote3*exp2nu*pdotxir*(-Btilde_prmS2z + expmu*expnu_prmS2z + expmu_prmS2z*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3*exp2nu*pdotxir*xisq_prmS2z*(Btilde - expmu*expnu)/(Deltat*sqrt(Q)*expmu*pow(xisq, 2)) + Sdote3*exp2nu*pdotxir_prmS2z*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - Sdote3*exp2nu*expmu_prmS2z*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*pow(expmu, 2)*xisq) + Sdote3*exp2nu_prmS2z*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) + Sdote3_prmS2z*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*sqrt(Q)*expmu*xisq) - 1.0/2.0*Q_prmS2z*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(Deltat*pow(Q, 3.0/2.0)*expmu*xisq) - Deltat_prmS2z*Sdote3*exp2nu*pdotxir*(-Btilde + expmu*expnu)/(pow(Deltat, 2)*sqrt(Q)*expmu*xisq);
