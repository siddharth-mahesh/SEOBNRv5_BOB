...
double ham_p1_deriv(double m1,double m2,int tortoise,double x,double y,double z,double p1,double p2,double p3,double S1x,double S1y,double S1z,double S2x,double S2y,double S2z){
    ...
    double HsoTerm2coeff_prmp1 = expnu*(-Q_prmp1 - 1.0/2.0*Q_prmp1/sqrt(Q))/(pow(Btilde, 2)*exp2mu*xisq*pow(sqrt(Q) + Q, 2)) - expnu*xisq_prmp1/(pow(Btilde, 2)*exp2mu*pow(xisq, 2)*(sqrt(Q) + Q)) + expnu_prmp1/(pow(Btilde, 2)*exp2mu*xisq*(sqrt(Q) + Q)) - exp2mu_prmp1*expnu/(pow(Btilde, 2)*pow(exp2mu, 2)*xisq*(sqrt(Q) + Q)) - 2*Btilde_prmp1*expnu/(pow(Btilde, 3)*exp2mu*xisq*(sqrt(Q) + Q));
