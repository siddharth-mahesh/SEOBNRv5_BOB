...
def dRvec(m1,m2,tortoise,x,y,z,p1,p2,p3,S1x,S1y,S1z,S2x,S2y,S2z):
    ...
    for i in range(3):
        for j in range(3):
            Tij = kron_delta(i,j) + q[i]*q[j]*xi_a_minusone*R2inv
            dR[i] += Tij*dHdP[j]
    return dR
...
def CalcOmega(m1,m2,tortoise,q,p,S1,S2):
    ...
    #print("S2 = ", S2)
    eta = m1*m2/(m1 + m2)/(m1 + m2)
    qdot = dRvec(m1,m2,tortoise,q[0],q[1],q[2],p[0],p[1],p[2],S1[0],S1[1],S1[2],S2[0],S2[1],S2[2])
    #print("qdot = ", qdot)
    ...
    dHprime = ham_first_derivs(m1,m2,tortoise,qprime[0],qprime[1],qprime[2],pprime[0],pprime[1],pprime[2],S1prime[0],S1prime[1],S1prime[2],S2prime[0],S2prime[1],S2prime[2])
    dHdpz = dHprime[5]
    dHdpy = dHprime[4]
