...
def new_compute_dHdx(m1, m2, eta, x, y, z, p1, p2, p3, S1x, S1y, S1z, S2x, S2y, S2z, KK, k0, k1, dSO, dSS, tortoise, EMgamma):
    ...
    Brtildeprm_x = (np.sqrt(Deltar)*Deltatprm_xprm_x - 2*Deltatprm_x + Deltarprm_x*Deltatprm_x/(2*np.sqrt(Deltar)))/(2*np.sqrt(Deltar*Deltat)) + (np.sqrt(Deltar)*Deltatprm_x - 2*Deltat)*(-Deltar*Deltatprm_x/2 - Deltarprm_x*Deltat/2)/(2*Deltar*Deltat*np.sqrt(Deltar*Deltat))
    ...
    Lambtprm_xprm_x = -2*Deltatprm_x*a**2*sin2thetaprm_x - 2*Deltatprm_xprm_x*a**2*sin2theta + 8*r**2*rprm_x + rprm_x*(4*a**2 + 4*r**2)
    ...
    nurprm_x = rprm_x/Sigma - Sigmaprm_x*r/Sigma**2 + w2*(-4*Deltat*rprm_x - 4*Deltatprm_x*r + Deltatprm_x*w2prm_x + Deltatprm_xprm_x*w2)/(2*Deltat*Lambt) + w2prm_x*(-4*Deltat*r + Deltatprm_x*w2)/(2*Deltat*Lambt) - Lambtprm_x*w2*(-4*Deltat*r + Deltatprm_x*w2)/(2*Deltat*Lambt**2) - Deltatprm_x*w2*(-4*Deltat*r + Deltatprm_x*w2)/(2*Deltat**2*Lambt)
    omegarprm_x = -Lambtprm_xprm_x*omegatilde/Lambt**2 + Lambtprm_x*(-2*Lambt*omegatildeprm_x + 2*Lambtprm_x*omegatilde)/Lambt**3
    ...
    SdotSkerrprm_x = S1prm_x*Skerr1 + S2prm_x*Skerr2 + S3prm_x*Skerr3
