...
def v5HM_BOB_optimized_merger_ringdown(t,t0,hNR,omegaNR,omegaQNM,tau):
    ...
    phi = omegaQNM*tau*(-np.atan2((1.0/2.0)*omegaNR, (1.0/2.0)*omegaQNM) + np.atan2(tmp10, (1.0/2.0)*omegaQNM)) + 0.5*omegaQNM*tau*np.log((1 - tmp2)*(tmp15 + 1)/((1 - tmp15)*(tmp2 + 1))) - tmp13*(-np.atan2((1.0/2.0)*omegaNR, tmp12) + np.atan2(tmp10, tmp12)) - 0.5*tmp13*np.log((1 - tmp16)*(tmp17 + 1)/((1 - tmp17)*(tmp16 + 1)))
    return h,phi