...
class SEOBNRv5_aligned_spin_waveform_quantities:
    ...
    def __init__(self) -> None:
        ...
        """
        m1, m2, self.r, self.phi, self.prstar, self.pphi, self.chi1, self.chi2, self.Hreal, self.Omega, self.Omega_circ = sp.symbols(
        ...
        self.noneqcond = max_delta_1em14 / (delta - 1e-14 + TINYDOUBLE)
        self.eqcond = min_delta_1em14 /(delta - 1e-14 - TINYDOUBLE)
        self.deltainvertible = delta * noneqcond + 1 * eqcond
        self.deltainv = 1 / deltainvertible
        ...
        self.eulerlog_vsq = []
        for m in range(1,9):
            self.eulerlog.append(sp.EulerGamma + sp.log(2 * m * self.vomega))
            self.eulerlog_vsq.append(sp.EulerGamma + sp.log(2 * m * self.vomega**2))
        
        self.khat = []
        for m in range(1,9):
            self.khat.append(m * self.Omega * self.Hreal)
        self.delta["(5 , 5)"] = self.vh3 * (
        ...
        self.fspin["(4 , 3)"] = self.vomega * (
            sp.Rational(5, 2) * self.nu * self.chi_S 
            - sp.Rational(5, 2) * self.nu * self.chi_A * self.deltainv
            ) / (1 - 2 * self.nu)
            +self.vomega**3 * (
                (
                    sp.Rational(887, 44) * self.nu 
                    - sp.Rational(3143, 132) * self.nu**2
                ) * self.chi_A * self.deltainv
                + (
                    -sp.Rational(529, 132) * self.nu**2 
                    - sp.Rational(667, 44) * self.nu
                ) * self.chi_S
            ) / (1 - 2 * self.nu)
            +self.vomega**4 * (
                (
                    12 * self.nu**2 
                    - sp.Rational(37, 3) * self.nu 
                    + sp.Rational(3, 2)
                ) * self.chi_A**2
                + (
                    sp.Rational(137, 6) * self.nu**2 
                    - 18 * self.nu 
                    + 3
                ) * self.chi_A * self.chi_S * self.deltainv
                + (
        ...
        self.fspin_limit["(4 , 3)"] = self.vomega * (- sp.Rational(5, 2) * self.nu * self.chi_A) / (1 - 2 * self.nu)
            + self.vomega**3 * (
                    sp.Rational(887, 44) * self.nu 
                    - sp.Rational(3143, 132) * self.nu**2
            ) * self.chi_A / (1 - 2 * self.nu)
            + self.vomega**4 * (
                sp.Rational(137, 6) * self.nu**2 
                - 18 * self.nu 
                + 3
            ) * self.chi_A * self.chi_S / (1 - 2 * self.nu)
        self.fspin["(3 , 1)"] = (
        ...
        self.fspin_limit["(3 , 3)"] = ( (sp.Rational(19, 2) * self.nu - 2) * self.chi_A) * self.vomega**3
        ...
        self.fspin["(2 , 1)"] = -sp.Rational(3, 2) * (
        ...
        self.fspin_limit["(2 , 1)"] = -sp.Rational(3, 2) * (self.chi_A) * self.vomega
            + (
                (
                    sp.Rational(131, 84) * self.nu 
                    + sp.Rational(61, 12)
        ...
        self.rho["(7 , 7)"] = 1 + 
            ...
            (714 * (3 * self.nu**2 - 4 * self.nu + 1))) * vomega**2
        
        self.rho["(7 , 6)"] = 1 + 
            ((6104 * self.nu**4 - 29351 * self.nu**3 + 37828 * self.nu**2 - 16185 * self.nu + 2144) /
            (1666 * (7 * self.nu**3 - 14 * self.nu**2 + 7 * self.nu - 1))) * self.vomega**2
        
        self.rho["(7 , 5)"] = 1 + 
            ((804 * self.nu**3 - 3523 * self.nu**2 + 3382 * self.nu - 762) /
            (714 * (3 * self.nu**2 - 4 * self.nu + 1))) * self.vomega**2
        
        self.rho["(7 , 4)"] = 1 + 
            ((41076 * self.nu**4 - 217959 * self.nu**3 + 298872 * self.nu**2 - 131805 * self.nu + 17756) /
            (14994 * (7 * self.nu**3 - 14 * self.nu**2 + 7 * self.nu - 1))) * self.vomega**2
        ...
        self.rho["(5 , 4)"] = 1 + 
            ...
            (13650 * (5 * self.nu**2 - 5 * self.nu + 1))) * self.vomega**2 - 
            sp.Rational(16213384, 15526875) * self.vomega**4
        
        self.rho["(5 , 3)"] = 1 + 
            ((176 * self.nu**2 - 850 * self.nu + 375) / (390 * (2 * self.nu - 1))) * self.vomega**2 - 
        ...
        self.rho["(4 , 2)"] = (
            ...
            + self.vomega ** 3 * (
                (
                    chi_A * (sp.Rational(10 , 15) - sp.Rational(21 , 15) * self.nu) * delta 
                    + chi_S * (sp.Rational(10 , 15) - sp.Rational(59 , 15) * self.nu + sp.Rational(78 , 15) * self.nu**2)
                ) / (-1 + 3*self.nu)
            )
            + self.vomega ** 4 * (
                (
        ...
        rho["(4 , 3)"] = 1 + 
            self.vomega**2 / (1 - 2 * nu) * (-sp.Rational(10, 11) * self.nu**2 + sp.Rational(547, 176) * self.nu - sp.Rational(111, 88)) - 
        ...
        rho44 = (
            ...
            + self.vomega**5 * (
                ...
                sp.Rational(1, (1 - 3 * self.nu)**2) * self.chi_S +
        ...
        self.rho["(3 , 2)"] = 1 
            ...
            + self.vomega**2 * (
                ...
                -(16 * self.nu**2 * chi_S**2) / (9 * (1 - 3 * self.nu)**2)
            ) 
            + self.vomega**3 * (
                (sp.Rational(13, 9) * self.nu + sp.Rational(2, 9)) * (self.delta * self.chi_A) / (1 - 3 * self.nu) 
                + (sp.Rational(607, 81) * self.nu**3 + sp.Rational(503, 81) * self.nu**2 - sp.Rational(1478, 405) * self.nu + sp.Rational(2, 9))* chi_S / (1 - 3 * self.nu)**2 
                + (320 * self.nu**3 * chi_S**3) / (81 * (1 - 3 * self.nu)**3)
            )
            + self.vomega**4 * (
        ...
        self.rho["(3 , 3)"] = 1 + 
            ...
            self.vomega**6 * (sp.Rational(3203101567, 227026800) + (-sp.Rational(129509, 25740) + sp.Rational(41, 192) * sp.pi**2) * self.nu -
                        sp.Rational(274621, 154440) * self.nu**2 + sp.Rational(12011, 46332) * self.nu**3 - sp.Rational(26, 7) * eulerlog3) + 
        ...
        self.rho["(2 , 2)"] = 1 + 
            ...
            vomega**3 * (
                 (-2 * (self.chi_S + self.chi_A * self.delta - self.chi_S * self.nu)) / 3.0
            ) +
            self.vomega**4 * (
        ...
        self.c = {}
        for k in range(2,10):
            self.c[f"{k}"] = sp.Rational(1 , 2**(k - 1)) * ((1 - delta)**(k - 1) + ((-1)**k) * (1 + delta)**(k - 1))
        self.c["3"] += -1 * eqcond
        self.c["5"] += sp.Rational(-1 , 2) * eqcond
        self.r0 = 2 / sp.sqrt(sp.E)
       self.effective_source = [] 
        self.effective_souce.append(Heff)
    ...
    def flux(self) -> sp.Basic:
        ...
        factorized_flux = 0
                ...
                n = (1 - (l + m) % 2) * (sp.Rational(8 , sp.factorial2(2 * l + 1)) * sp.pi * (m)**l * sp.sqrt(sp.Rational((l + 1)*(l + 2) , l * (l - 1))))
                    + ((l + m) % 2 ) * (sp.Rational(16 , sp.factorial2(2 * l + 1)) * sp.pi * (m)**l * sp.sqrt(sp.Rational((l**2 -  m**2) * (l + 1) * (2 * l + 1) , (2 * l - 1) * (l + 1) * l * (l - 1))))
                    tail_term = sp.exp(khat[m] * sp.pi) / self.factorial[l]
                    b = 2 * khat[m]
        ...
        factorized_flux *= - (sp.Rational(1,8) * self.Omega / sp.pi)        return factorized_flux
    def strain(self) -> sp.Basic:
        ...
        hlms = {}
        for l in range(2 , 5):
            for m in range(l , max(l-2,4), -1):
                pn_contribution_f = self.noneqcond * self.rho[f"({l} , {m})"]**l
                if not (l + m) % 2:
                    pn_contribution_f += self.noneqcond * self.fspin[f"({l} , {m})"] + self.eqcond * self.fspin_limit[f"({l} , {m})"]
                if l == 3 and m == 3:
                    pn_contribution_f += self.noneqcond * self.fspinimag + self.eqcond * self.fspinimag_limit
                n = (1 - (l + m) % 2) * (sp.Rational(8 , sp.factorial2(2 * l + 1)) * sp.pi * (sp.I * m)**l * sp.sqrt(sp.Rational((l + 1)*(l + 2) , l * (l - 1))))
                    + ((l + m) % 2 ) * (sp.Rational(-16 , sp.factorial2(2 * l + 1)) * sp.pi * (sp.I * m)**l * sp.sqrt(sp.Rational((l**2 -  m**2) * (l + 1) * (2 * l + 1) , (2 * l - 1) * (l + 1) * l * (l - 1))))
                   tail_term = (
                       sp.gamma(l + 1 - 2 * sp.I * self.khat[m]) 
                       * sp.exp(self.khat[m] * (sp.pi + 2 * sp.I * sp.log(2 * m * self.Omega * self.r0)))
                       / sp.factorial(l)
                   )
                hlms[f"{l},{m}"] = newtonian_strain * self.effective_source[(l + m) % 2] * tail_term * pn_contribution_f * delta[l,m]
        return hlms
