#include "SEOBNRBOB.h"
int v5HM_BOB_optimized_waveform(double complex *h22,double m1, double m2, double r, double phi, double prstar, double pphi, double chi1, double chi2, double Hreal, double Omega, double Omega_circ){
const double complex tmp0 = ((Omega)*(Omega));
const double complex tmp1 = m1 + m2;
const double complex tmp5 = pow(Omega, 8.0/3.0);
const double complex tmp6 = log(4*cbrt(Omega));
const double complex tmp7 = pow(Omega, 10.0/3.0);
const double complex tmp14 = pow(Omega, 5.0/3.0);
const double complex tmp20 = ((M_PI)*(M_PI));
const double complex tmp23 = Hreal*Omega;
const double complex tmp10 = (1.0/2.0)*chi1 + (1.0/2.0)*chi2;
const double complex tmp11 = (1.0/2.0)*chi1 - 1.0/2.0*chi2;
const double complex tmp12 = (m1 - m2)/tmp1;
const double complex tmp15 = ((m1)*(m1))*((m2)*(m2))/((tmp1)*(tmp1)*(tmp1)*(tmp1));
const double complex tmp21 = ((m1)*(m1)*(m1))*((m2)*(m2)*(m2))/pow(tmp1, 6);
const double complex tmp24 = 4*I*tmp23;
const double complex tmp3 = (1.0/((tmp1)*(tmp1)));
const double complex tmp13 = tmp11*tmp12;
const double complex tmp16 = ((tmp10)*(tmp10));
const double complex tmp17 = ((tmp11)*(tmp11));
const double complex tmp4 = m1*m2*tmp3;
const double complex EulerGamma = 0.57721566490153286;
