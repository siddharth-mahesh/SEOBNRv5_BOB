...
def output_H_and_derivs():
    ...
    f = open("Schwarzschild/SymPy_Hreal_Sph_on_bottom.txt", 'r')
...
#        outsprhs.append(rhss_deriv_p3[i])
    ...
    with open("Playground_Pycodes/new_dHdr.py", "w") as file:
...
""")
    ...
    with open("Playground_Pycodes/new_dHdptheta.py", "w") as file:
        file.write("""from __future__ import division
import numpy as np
...
""")
    ...
    with open("Playground_Pycodes/new_dHdpphi.py", "w") as file:
        file.write("""from __future__ import division
import numpy as np
...
""")
    ...
    with open("Playground_Pycodes/dHdr.txt", "w") as file:
        for i in range(len(lr)):
    ...
    with open("Playground_Pycodes/dHdpphi.txt", "w") as file:
        for i in range(len(lr) - 1):
            file.write(lr[i].lhs + " = " + str(lr[i].rhs) + "\n")
        for i in range(len(lhss_deriv_pphi)):
