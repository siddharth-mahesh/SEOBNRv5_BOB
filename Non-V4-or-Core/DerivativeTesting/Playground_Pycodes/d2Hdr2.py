...
def compute_d2Hdr2(m1, m2, eta, r, theta, phi, pr, ptheta, pphi, S1r, S1theta, S1phi, S2r, S2theta, S2phi, KK, k0, k1, dSO, dSS, tortoise, EMgamma):
    ...
    Hrealprm_r = prTprm_r
    u_rprm_r = 2/r**3
    prT_rprm_r = 2*pphi**2*u*u_rprm_r/sintheta + 2*pphi**2*u_r*uprm_r/sintheta + 2*ptheta**2*u*u_rprm_r + 2*ptheta**2*u_r*uprm_r
    Hreal_rprm_r = prT_rprm_r
    return np.array([Hreal,Hreal_rprm_r])